...
- Generic LLM features use the server-only Platform AI bridge ONLY. Never ask
  ...
  Use `generatePlatformAIImage({ scene_key, prompt, n: 1, size })` for image
  generation; it returns the first generated image URL.
...
- `services/platform-ai.ts` — server-only Platform AI chat bridge:
  ...
  or `text` / `image_url` parts. `requestPlatformAIImage({ sceneKey, prompt, n,
  size })` calls the image-generation proxy and returns only its first URL.
...
- `api.ts` — `apiFetch(path, init?: RequestInit & { auth?: boolean; silent?: boolean })` → `Response`.
  ...
  `PlatformAIChatMessage`, `PlatformAIChatInput`, `PlatformAIImageInput`, and
  ...
  `image_url` parts. `generatePlatformAIImage({ scene_key, prompt, n: 1, size })`
  returns the generated image URL. Also exports
