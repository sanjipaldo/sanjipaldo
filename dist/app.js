...
const naverCategoryTree = {
  ...
  "패션잡화": {
    "여성가방": { "숄더백": ["미니백", "크로스백"], "토트백": ["에코백", "캔버스백"], "백팩": ["미니백팩", "노트북백팩"], "클러치": ["파우치", "이브닝클러치"] },
    "남성가방": { "백팩": ["노트북백팩", "데일리백팩"], "서류가방": ["브리프케이스", "토트형서류가방"], "크로스백": ["메신저백", "슬링백"] },
    "신발": { "스니커즈": ["캔버스화", "러닝화", "단화"], "구두": ["로퍼", "옥스포드", "더비슈즈"], "부츠": ["앵클부츠", "롱부츠"], "샌들/슬리퍼": ["플립플롭", "슬라이드"] },
    "패션소품": { "벨트": ["가죽벨트", "버클벨트"], "지갑": ["카드지갑", "장지갑", "반지갑"], "스카프/머플러": ["실크스카프", "니트머플러"] },
    "주얼리/시계": { "목걸이": ["펜던트목걸이", "체인목걸이"], "반지": ["커플링", "미니멀링"], "시계": ["손목시계", "스마트워치밴드"] },
  ...
  "화장품/미용": {
    ...
    "헤어케어": { "샴푸/린스": ["탈모샴푸", "두피케어샴푸"], "헤어스타일링": ["왁스", "에센스", "헤어오일"], "염색/펌": ["새치염색약", "헤어매니큐어"] },
    "바디케어": { "바디워시": ["바디워시", "입욕제"], "바디로션": ["바디로션", "핸드크림"], "제모/왁싱": ["제모기", "왁싱스트립"] },
    "향수": { "여성향수": ["오드퍼퓸", "오드뜨왈렛"], "남성향수": ["오드코롱", "애프터쉐이브"] },
    "미용기기": { "클렌징기기": ["클렌징브러시", "초음파클렌저"], "피부관리기기": ["LED마스크", "갈바닉기기"] }
  },
  "디지털/가전": {
    ...
    "TV/영상가전": { "TV": ["OLED TV", "QLED TV"], "프로젝터": ["빔프로젝터", "미니빔프로젝터"] },
    "생활가전": { "주방가전": ["에어프라이어", "전자레인지", "커피머신"], "계절가전": ["에어컨", "선풍기", "히터"], "청소가전": ["로봇청소기", "무선청소기"] },
    "카메라/캠코더": { "디지털카메라": ["미러리스", "DSLR"], "액션캠": ["고프로", "짐벌"], "렌즈/액세서리": ["카메라렌즈", "삼각대"] },
    "게임기": { "콘솔게임기": ["플레이스테이션", "닌텐도스위치"], "게임타이틀": ["액션게임", "RPG게임"] }
  },
  "가구/인테리어": {
    "침실가구": { "침대": ["프레임", "매트리스"], "옷장": ["붙박이장", "드레스룸", "행거"], "화장대": ["거울화장대", "화장대의자"] },
    "거실가구": { "소파": ["패브릭소파", "가죽소파", "리클라이너"], "테이블": ["거실테이블", "좌식테이블"], "TV장": ["거실장", "콘솔장"] },
    ...
    "조명": { "실내조명": ["LED조명", "팬던트조명"], "무드등": ["무드등", "취침등"], "스탠드": ["독서스탠드", "LED스탠드"] },
    "DIY자재/부자재": { "벽지/시트지": ["포인트벽지", "합지벽지"], "바닥재": ["장판", "데코타일"] }
  },
  "출산/육아": {
    ...
    "유모차/카시트": { "유모차": ["절충형", "디럭스형", "휴대용"], "카시트": ["신생아용", "확장형"], "웨건/웨빙": ["다목적웨건", "웨빙캐리어"] },
  ...
  "생활/건강": {
    ...
    "문구/사무용품": { "필기구": ["볼펜", "연필", "만년필"], "사무용품": ["파일", "노트", "프린터용지"], "다이어리/플래너": ["스케줄러", "위클리플래너"] },
    "일회용품": { "위생용품": ["마스크", "장갑"], "주방일회용품": ["나무젓가락", "빨대", "랩"] },
    "건강관리용품": { "다이어트용품": ["체지방계", "폼롤러"], "금연/금주용품": ["금연보조제", "금연패치"] }
  },
  "여가/생활편의": {
    "도서": { "소설": ["국내소설", "해외소설"], "자기계발": ["에세이", "경제경영"], "어린이도서": ["그림책", "동화책"] },
    "티켓/공연": { "공연": ["콘서트", "뮤지컬", "연극"], "전시": ["미술전시", "체험전"], "스포츠관람": ["야구티켓", "축구티켓"] },
    "여행/항공권": { "항공권": ["국내선", "국제선"], "숙박": ["호텔", "펜션", "게스트하우스"], "투어/입장권": ["테마파크입장권", "워터파크입장권"] },
    "상품권": { "온라인상품권": ["문화상품권", "도서문화상품권"], "모바일상품권": ["기프티콘", "카카오페이상품권"], "백화점상품권": ["신세계상품권", "롯데상품권"] },
    "취미": { "음반/DVD": ["K팝음반", "OST음반"], "악기": ["기타", "피아노"], "핸드메이드": ["DIY키트", "자수키트"] },
    "자동차용품": { "카액세서리": ["방향제", "거치대"], "카케어": ["세차용품", "광택제"] }
  },
  "면세점": {
    "향수/화장품": { "향수": ["여성향수", "남성향수"], "스킨케어": ["에센스세트", "크림세트"], "메이크업": ["립스틱세트", "쿠션세트"] },
    "주류": { "위스키": ["싱글몰트", "블렌디드"], "와인": ["레드와인", "화이트와인"], "전통주": ["사케", "청주"] },
    "패션잡화": { "가방": ["명품가방", "여행가방"], "지갑": ["명품지갑", "카드지갑"], "벨트": ["명품벨트", "버클벨트"] },
    "선글라스/시계": { "선글라스": ["명품선글라스", "편광선글라스"], "시계": ["명품시계", "스마트워치"] },
    "건강식품": { "영양제": ["비타민세트", "오메가3세트"], "홍삼": ["홍삼정", "홍삼스틱"] }
  }
};
...
/* 예전 이름(쌀·잡곡·견과, 어류, 갑각류…)으로 저장된 카테고리를 네이버쇼핑식 이름으로 옮긴다. */
const LEGACY_CATEGORY_MOVES = { "건강식품|꿀·프로폴리스": ["건강식품", "꿀/로얄젤리"], "수산물|어류": ["수산물", "생선"], "수산물|갑각류": ["수산물", "게/대게"], "가공식품|김치·반찬": ["김치", "배추김치"], "가공식품|건조식품": ["농산물", "버섯"], "가공식품|즉석·간편식": ["냉동/간편조리식품", "즉석밥"], "농산물|쌀·잡곡·견과": ["농산물", "쌀/잡곡"], "건강식품|차·건강즙": ["음료", "차류"], "축산물|쇠고기": ["축산물", "소고기"] };
function migrateCategoryPath(item, keys) {
  const [groupKey, midKey, subKey, detailKey] = keys;
  const moved = LEGACY_CATEGORY_MOVES[`${item[midKey]}|${item[subKey]}`];
  const next = {};
  if (moved) { next[midKey] = moved[0]; next[subKey] = moved[1]; }
  if (item[detailKey] === "감귤") next[detailKey] = "감귤/한라봉";
  keys.forEach(key => { const value = next[key] ?? item[key]; if (typeof value === "string" && value.includes("·")) next[key] = value.replace(/·/g, "/"); });
...
function customsInputMarkup(value = "", required = true, hidden = false) {
  return `<div class="form-field full customs-input" data-customs-field ${hidden ? 'style="display:none"' : ""}><label>개인통관고유부호 * <small>해외직구 상품은 통관에 꼭 필요해요</small></label><input name="personalCustomsCode" value="${escapeHtml(value)}" maxlength="13" autocomplete="off" inputmode="text" placeholder="P로 시작하는 13자리 (예: P123456789012)" ${required && !hidden ? "required" : ""}><small class="field-hint">관세청 ‘개인통관고유부호’ 조회에서 확인할 수 있어요. 국내 상품은 입력하지 않아도 됩니다.</small></div>`;
}
function productBrand(product) { return product?.brand || product?.supplier || "브랜드"; }
function productCategoryPath(product) { return [product?.categoryGroup || "식품", product?.category, product?.categorySub, product?.categoryDetail].filter(Boolean); }

/* ===== 상품 옵션 (예: 감귤 로얄과 2kg / 3kg / 5kg …) =====
...
function syncProductOptionTotals(product) {
  ...
  product.supply = Math.min(...options.map(option => Number(option.supply || 0)));
  product.recommended = Math.min(...options.map(option => Number(option.recommended || option.supply || 0)));
  product.stock = options.reduce((sum, option) => sum + Number(option.stock || 0), 0);
  return product;
}
function productPriceLabel(product, key = "supply") {
  const options = productOptions(product);
  if (options.length > 1) return `${money(Math.min(...options.map(option => Number(option[key] || option.supply || 0))))}~`;
  return money(product?.[key] || 0);
}
function sellerOptionRows(item, product = productOf(item?.productId)) {
  const options = productOptions(product);
  if (!options.length) return [];
  const markup = Number(item?.salePrice || product?.recommended || 0) - Number(product?.recommended || product?.supply || 0);
  return options.map((option, index) => {
    const fallback = Math.max(Number(option.supply || 0), Number(option.recommended || option.supply || 0) + Math.max(0, markup));
    const salePrice = Number(item?.optionPrices?.[option.id] ?? fallback);
    return { ...option, index: index + 1, salePrice, enabled: item?.optionEnabled?.[option.id] !== false };
  });
}
function sellerItemPriceLabel(item, product = productOf(item?.productId)) {
  const rows = sellerOptionRows(item, product).filter(row => row.enabled);
  if (rows.length > 1) return `${money(Math.min(...rows.map(row => row.salePrice)))}~`;
  if (rows.length === 1) return money(rows[0].salePrice);
  return money(item?.salePrice || product?.recommended || 0);
}
function sellerItemMarginLabel(item, product = productOf(item?.productId)) {
  const rows = sellerOptionRows(item, product).filter(row => row.enabled);
  if (!rows.length) return `${margin(product?.supply || 0, item?.salePrice || product?.recommended || 0)}%`;
  const values = rows.map(row => margin(row.supply, row.salePrice));
  const low = Math.min(...values), high = Math.max(...values);
  return low === high ? `${low}%` : `${low}~${high}%`;
}
function sellerItemOptionSummary(item, product = productOf(item?.productId)) {
  const rows = sellerOptionRows(item, product);
...
const initialState = {
  ...
  sellerDashboard: { layout: [...DEFAULT_DASHBOARD_LAYOUT], layoutVersion: DASHBOARD_LAYOUT_VERSION },
  chatFavorites: {},
  partnerNotes: {},
  members: [
  ...
  products: [
    ...
    { id: "DF-3240", brand: "서해바다상회", imageIndex: 4, emoji: "🦀", name: "국내산 손질 꽃게 1kg", supplier: "산지팔도", supplierLoginId: "sup", supply: 18900, recommended: 26900, stock: 38, categoryGroup: "식품", category: "수산물", categorySub: "게/대게", categoryDetail: "꽃게", status: "판매중", imported: false, origin: "서해안", detail: "선별 손질 꽃게 · 아이스 포장 · 상세페이지 제공" },
...
let sellerCatalogSort = "recommend";
let pickStatusFilter = "all";
let onSaleSearch = "";
let onSaleDateFrom = "";
let onSaleDateTo = "";
let onSaleCategoryFilter = "전체";
...
function loadState() {
  try {
    ...
    if (merged.sellerDashboard.layoutVersion !== DASHBOARD_LAYOUT_VERSION) merged.sellerDashboard = { layout: [...DEFAULT_DASHBOARD_LAYOUT], layoutVersion: DASHBOARD_LAYOUT_VERSION };
    ...
    merged.products.forEach(product => {
      ...
      if (seed && !product.brand) product.brand = seed.brand;
    ...
    merged.sellerProducts.forEach(item => Object.assign(item, migrateCategoryPath(item, ["sellerCategoryGroup", "sellerCategory", "sellerCategorySub", "sellerCategoryDetail"])));
    /* v2 흐름: 승인 완료(내 상품 꾸미기) → 마스터 상품 등록. 이전에 승인된 상품은 이미 마스터 상품으로 본다. */
    if (Number(saved.flowVersion || 0) < 1) merged.sellerProducts.forEach(item => { if (item.approvalStatus === "승인완료" && item.masterRegistered === undefined) item.masterRegistered = true; });
    merged.flowVersion = 1;
    /* 옵션 MVP: 시드 상품(감귤 5종·김치 3종)에 옵션을 넣고, 이미 저장된 상품명/판매 상품명도 새 이름으로 맞춘다. */
    if (Number(saved.optionsVersion || 0) < 1) {
      ...
      merged.sellerProducts.forEach(item => {
        const seed = base.sellerProducts.find(entry => entry.id === item.id);
        const product = merged.products.find(entry => entry.id === item.productId);
        const legacy = base.products.find(entry => entry.id === item.productId)?.legacyName;
        if (legacy && item.customTitle === legacy) item.customTitle = seed?.customTitle || product?.name || item.customTitle;
        if (seed?.optionPrices && !item.optionPrices) { item.optionPrices = { ...seed.optionPrices }; item.optionEnabled = { ...(seed.optionEnabled || {}) }; item.salePrice = seed.salePrice; }
        Object.entries(item.channelDetails || {}).forEach(([channelId, detail]) => {
          if (detail.title === "달콤한 제주 하우스 감귤 3kg") detail.title = seed?.channelDetails?.[channelId]?.title || "달콤한 제주 하우스 감귤 2~5kg";
          else if (legacy && detail.title === legacy) detail.title = product?.name || detail.title;
        });
      });
    }
    merged.optionsVersion = 1;
    /* 송장 쇼핑몰 전송: 기본은 ‘쇼핑몰 전송’ 버튼으로 직접 보내고, 설정에서 자동 전송을 켤 수 있다. */
    if (Number(saved.automationVersion || 0) < 1) {
      (merged.channelConnections.seller || []).forEach(channel => { channel.trackingAutomation = false; });
    ...
    /* 요금제 개편: 무료(수기 주문) / 쇼핑몰 자동화 9,900원 / 카카오톡 알림 2,900원, 가입 즉시 이용 */
    if (Number(saved.planVersion || 0) < 1) {
      Object.values(merged.subscriptions || {}).forEach(sub => { if (!sub.tier) Object.assign(sub, { tier: "auto", plan: "쇼핑몰 자동화", monthlyFee: 9900, startedAt: sub.startedAt || "2026.08.08" }); });
      Object.values(merged.notificationServices || {}).forEach(notice => Object.assign(notice, { plan: "카카오톡 알림", monthlyFee: 2900, morningTime: notice.morningTime || "09:00", eveningTime: notice.eveningTime || "18:00", briefOn: notice.briefOn !== false, reportOn: notice.reportOn !== false }));
      (merged.members || []).forEach(member => { if (["pending", "profile-incomplete"].includes(member.status)) { member.status = "approved"; member.approvedAt = member.approvedAt || "가입 즉시 이용"; member.docsPending = !(member.businessFile && (member.bankbookFile || member.bankAccountNumber)); } });
      (merged.supplierApplications || []).forEach(application => {
        if (application.status !== "pending") return;
        application.status = "approved"; application.reviewedAt = "즉시 이용";
        const member = (merged.members || []).find(item => item.loginId === application.sellerLoginId);
...
function supplierAutoApprove(supplierLoginId) { return Boolean(state.supplierSettings?.[supplierLoginId]?.autoApprove); }
...
function approvePickRequest(item, auto = false) {
  ...
  item.approvedAt = auto ? "방금 전 · 자동 승인" : "방금 전";
  item.masterRegistered = false;
...
function sellerChannels(loginId = currentAccount?.loginId || "seller") {
  ...
  if (member && (member.roles || [member.role]).includes("seller") && loginId !== "seller") {
    state.channelConnections[loginId] = (state.channelConnections.seller || []).map(channel => ({ id: channel.id, name: channel.name, mark: channel.mark, color: channel.color, status: "disconnected", storeName: "연동 전", lastSync: "-", trackingAutomation: false, syncInterval: 10, lastTrackingPush: "-" }));
    return state.channelConnections[loginId];
  }
  return state.channelConnections.seller || [];
}
/* ===== 요금제 =====
...
const AUTOMATION_ACTIONS = ["connect-channel", "reconnect-channel", "collect-orders", "toggle-auto-collect", "toggle-auto-tracking", "toggle-channel-automation", "push-tracking", "push-all-tracking", "run-tracking-sync", "manage-product-channels", "sync-all-channels", "sync-channel-listing", "push-channel-listing", "refresh-channel-policies"];
...
function memberDocsModal() {
  ...
  if (!member) return;
  openModal(`<h2>정산 계좌 · 사업자 서류 등록</h2><p>지금 바로 두고를 쓸 수 있어요. 정산·환불 돈을 받으려면 계좌와 서류를 등록해 주세요.</p>
    <form id="memberDocsForm" class="form-grid" novalidate>
      <div class="form-field full"><label>사업자등록증 ${member.businessFile ? `<small>제출됨 · ${escapeHtml(member.businessFile)}</small>` : "*"}</label><label class="signup-file"><input name="businessFile" type="file" accept="image/*,.pdf" data-signup-file><span data-file-label>${member.businessFile ? "✓ 다시 올리려면 누르세요" : "📄 사진 찍기 또는 파일 선택"}</span></label></div>
      <div class="form-field"><label>정산·환불 은행 *</label><select name="bankName"><option value="">은행 선택</option>${REFUND_BANKS.map(bank => `<option ${member.bankName === bank ? "selected" : ""}>${bank}</option>`).join("")}</select></div>
      <div class="form-field"><label>예금주 *</label><input name="bankAccountHolder" value="${escapeHtml(member.bankAccountHolder || "")}" placeholder="상호 또는 대표자명"></div>
      <div class="form-field full"><label>계좌번호 *</label><input name="bankAccountNumber" inputmode="numeric" value="${escapeHtml(member.bankAccountNumber || "")}" placeholder="- 없이 숫자만"></div>
      <div class="form-field full"><label>통장 사본 ${member.bankbookFile ? `<small>제출됨 · ${escapeHtml(member.bankbookFile)}</small>` : "*"}</label><label class="signup-file"><input name="bankbookFile" type="file" accept="image/*,.pdf" data-signup-file><span data-file-label>${member.bankbookFile ? "✓ 다시 올리려면 누르세요" : "📄 사진 찍기 또는 파일 선택"}</span></label></div>
      <div class="modal-actions full"><button type="button" class="secondary-button" data-close-modal>나중에</button><button type="submit" class="primary-button">등록하기</button></div>
    </form>`);
}
...
function morningBriefMessage(loginId = currentAccount?.loginId) {
  ...
  const balance = state.deposits?.[loginId]?.balance ?? 0;
  const lines = [["신규 주문", newOrders], ["결제하면 공급사로 가는 주문", payWait], ["상품 연결이 필요한 주문", mapWait], ["쇼핑몰에 보낼 송장", trackingWait], ["쇼핑몰 확인이 필요한 주문", checkWait], ["공급가 변경 알림", priceAlerts]].filter(([, count]) => count > 0);
  const total = lines.reduce((sum, [, count]) => sum + count, 0);
  return { title: "☀️ 오늘 아침 두고 브리핑", body: `좋은 아침이에요, ${member?.company || "사장님"} 사장님!\n${total ? `오늘 처리할 일이 ${total}건 있어요.` : "오늘은 급하게 처리할 일이 없어요. 🙌"}\n${lines.map(([label, count]) => `• ${label} ${count}건`).join("\n")}${lines.length ? "\n" : ""}\n💰 예치금 잔액 ${money(balance)}${balance < 50000 ? " (충전이 필요해요)" : ""}`, button: "두고에서 바로 처리하기" };
}
function eveningReportMessage(loginId = currentAccount?.loginId) {
  const member = memberByLogin(loginId) || currentAccount;
  ...
  const diff = today.amount - yesterday.amount;
  const rate = yesterday.amount ? Math.round((diff / yesterday.amount) * 100) : null;
  const trend = !today.amount && !yesterday.amount ? "오늘은 아직 매출이 없어요. 내일은 더 좋아질 거예요!" : rate === null ? "첫 매출이 생겼어요! 🎉" : diff >= 0 ? `어제보다 ${money(diff)} (▲${rate}%) 늘었어요 📈` : `어제보다 ${money(-diff)} (▼${Math.abs(rate)}%) 줄었어요`;
  const month = dateKeyOffset(0).slice(0, 7);
  const monthSales = mine.filter(order => String(order.orderDate || "").startsWith(month) && !["취소완료", "환불완료"].includes(order.status)).reduce((sum, order) => sum + Number(order.amount || 0), 0);
  return { title: "🌙 오늘 마감 리포트", body: `${member?.company || "사장님"} 사장님, 오늘도 수고 많으셨어요.\n\n📦 오늘 주문 ${today.count}건 · 매출 ${money(today.amount)}\n${trend}\n✅ 결제·발주 ${paidToday}건 · 출고 ${shipped}건\n${leftover ? `⚠️ 내일 처리할 주문 ${leftover}건이 남았어요` : "✨ 남은 주문 없이 깔끔하게 마감했어요"}\n\n이번 달 누적 매출 ${money(monthSales)}`, button: "오늘 매출 자세히 보기" };
}
function kakaoBubble(message, time) {
  return `<div class="kakao-preview"><div class="kakao-preview-head"><span class="kp-avatar">두</span><b>두고</b><small>${escapeHtml(time)}</small></div><div class="kakao-preview-bubble"><div class="kp-top">알림톡 도착</div><b>${escapeHtml(message.title)}</b><p>${escapeHtml(message.body).replace(/\n/g, "<br>")}</p><span class="kp-btn">${escapeHtml(message.button)}</span></div></div>`;
}
...
function autoTrackingPushEnabled(loginId = currentAccount?.loginId || "seller") {
  const connected = sellerChannels(loginId).filter(channel => channel.status === "connected");
  return connected.length > 0 && connected.every(channel => channel.trackingAutomation);
}
...
function processDueTrackingPushes(now = Date.now()) {
  ...
  (state.orders || []).forEach(order => {
    ...
    Object.entries(order.channelTrackingStatuses || {}).forEach(([channelId, status]) => {
      ...
      if (!sellerAutomationActive(order.sellerLoginId)) return;
      const channel = sellerChannels(order.sellerLoginId).find(item => item.id === channelId);
...
function pushOrderTracking(order, mode = "manual") {
  ...
  Object.entries(order?.channelTrackingStatuses || {}).forEach(([channelId, status]) => {
    ...
    sentTo.push(channelMeta(channelId).name);
  });
  if (sentTo.length) { order.trackingPushedAt = "방금 전"; order.channelTrackingSyncedAt = "방금 전"; }
  return sentTo;
}
function pushAllPendingTracking(mode = "manual") {
...
function runSellerTrackingSync(showFeedback = true) {
  ...
  saveState();
  if (showFeedback) showToast(result.count ? `송장 ${result.count}건을 ${result.channels.join(", ")}에 보냈어요.` : "쇼핑몰에 보낼 송장이 없어요. 모두 전송됐어요.");
  return result.count;
}

/* ===== 쇼핑몰 주문 자동 수집 (데모) =====
   두고에서 전송한 상품(옵션 포함)에 들어온 주문을 쇼핑몰에서 가져온다. 두고 상품이라 매핑 없이 바로 결제 단계로 간다. */
const SAMPLE_BUYERS = [
  { name: "김민지", phone: "010-2481-3390", postalCode: "04524", address: "서울특별시 중구 세종대로 110", detail: "301호", message: "문 앞에 놓아주세요." },
  { name: "박서준", phone: "010-7732-1180", postalCode: "48058", address: "부산광역시 해운대구 센텀중앙로 90", detail: "1502호", message: "경비실에 맡겨주세요." },
  { name: "이하은", phone: "010-5520-9912", postalCode: "34126", address: "대전광역시 유성구 대학로 99", detail: "101동 704호", message: "배송 전 연락 부탁드려요." },
  { name: "최도윤", phone: "010-3308-4471", postalCode: "61475", address: "광주광역시 동구 금남로 245", detail: "2층", message: "부재 시 문 앞에 두세요." },
  { name: "정서연", phone: "010-9051-2264", postalCode: "63309", address: "제주특별자치도 제주시 연삼로 473", detail: "", message: "선물 포장 부탁드려요." }
];
function sellerAutomationSettings(loginId = currentAccount?.loginId || "seller") {
  state.sellerSettings = state.sellerSettings || {};
...
function showSignup(role = "seller", returnTarget = "seller") {
  ...
  signupFormEl.reset();
  signupFormEl.classList.remove("docs-later");
  signupFormEl.querySelectorAll("[data-file-label]").forEach(label => { label.textContent = "📄 사진 찍기 또는 파일 선택"; label.closest(".signup-file").classList.remove("has-file"); });
  signupFormEl.querySelector("[data-business-result]").hidden = true;
  signupFormEl.querySelector("[data-phone-code-row]").hidden = true;
  signupPhoneCode = "";
  fillBankSelect(signupFormEl);
  ...
  document.getElementById("signupDescription").textContent = normalizedRole === "supplier" ? "4단계로 입점을 신청해요. 사업자 서류는 나중에 올려도 돼요." : "사업자라면 4단계로 가입해요. 서류는 나중에 올려도 돼요.";
  ...
  setSignupStep(restoreSignupDraft(normalizedRole));
  window.scrollTo({ top: 0 });
}

function setSignupStep(step) {
  signupStep = Math.max(1, Math.min(4, Number(step) || 1));
...
function validateSignupStep(step) {
  ...
  const fail = (message, name) => { if (error) error.textContent = message; const field = name && form.elements[name]; if (field?.focus) field.focus(); return false; };
  if (step === 1) {
    const email = String(form.elements.email.value || "").trim().toLowerCase();
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return fail("이메일 주소를 확인해 주세요.", "email");
    if (getAccount(email)) return fail("이미 가입된 이메일이에요. 로그인하거나 비밀번호 찾기를 이용해 주세요.", "email");
    const password = form.elements.password.value;
    if (password.length < 8 || !/[A-Za-z]/.test(password) || !/\d/.test(password)) return fail("비밀번호는 영문과 숫자를 섞어 8자 이상으로 만들어 주세요.", "password");
    if (password !== form.elements.passwordConfirm.value) return fail("비밀번호가 서로 달라요.", "passwordConfirm");
    if (!form.elements.phoneVerified.value) return fail("휴대폰 인증을 해 주세요. ‘인증번호 받기’를 누르면 돼요.", "contact");
  }
  if (step === 2) {
    if (!isValidBusinessNo(form.elements.businessNo.value)) return fail("사업자등록번호 10자리를 확인해 주세요.", "businessNo");
    if (!form.elements.businessVerified.value) return fail("‘사업자 진위 확인하기’를 눌러 사업자 확인을 해 주세요.");
    if (!form.elements.company.value.trim()) return fail("상호명을 넣어 주세요.", "company");
  }
  if (step === 3 && !form.elements.docsLater.checked) {
    if (!signupFileName(form.elements.businessFile)) return fail("사업자등록증을 올려 주세요. 지금 없으면 아래 ‘나중에 올리기’를 체크해요.");
    if (!form.elements.bankName.value) return fail("정산·환불 받을 은행을 골라 주세요.", "bankName");
    if (!form.elements.bankAccountHolder.value.trim()) return fail("예금주를 넣어 주세요.", "bankAccountHolder");
    if (form.elements.bankAccountNumber.value.replace(/\D/g, "").length < 8) return fail("계좌번호를 확인해 주세요.", "bankAccountNumber");
    if (!signupFileName(form.elements.bankbookFile)) return fail("통장 사본을 올려 주세요.");
  }
  if (step === 4) {
    if (!String(form.elements.referralCode.value || "").trim()) return fail("추천인 코드는 필수예요.", "referralCode");
    if (!form.elements.terms.checked || !form.elements.privacy.checked) return fail("필수 약관 2개에 동의해 주세요.");
  }
...
function productMatchesCategory(product, path) {
  const [group, mid, sub, detail] = path;
  return (!group || group === "전체" || (product.categoryGroup || "식품") === group)
    && (!mid || mid === "전체보기" || product.category === mid)
    && (!sub || sub === "전체보기" || product.categorySub === sub)
    && (!detail || detail === "전체보기" || product.categoryDetail === detail);
}
function sellerCatalogProducts() {
  const path = [sellerCategoryGroup, sellerCategory, sellerCategorySub, sellerCategoryDetail];
  const list = catalogBaseProducts().filter(product => productMatchesCategory(product, path));
  if (sellerCatalogSort === "margin") return [...list].sort((a, b) => margin(b.supply, b.recommended) - margin(a.supply, a.recommended));
  if (sellerCatalogSort === "supply") return [...list].sort((a, b) => Number(a.supply) - Number(b.supply));
  return list;
}
...
const NAVER_CATEGORY_ICONS = { "패션의류": "👗", "패션잡화": "👜", "화장품/미용": "💄", "디지털/가전": "💻", "가구/인테리어": "🛋️", "출산/육아": "🍼", "식품": "🍎", "스포츠/레저": "⚽", "생활/건강": "🧴", "여가/생활편의": "🎟️", "면세점": "✈️" };
function selectedMarketCategoryPath() {
  return [sellerCategoryGroup === "전체" ? "" : sellerCategoryGroup, sellerCategory === "전체보기" ? "" : sellerCategory, sellerCategorySub === "전체보기" ? "" : sellerCategorySub, sellerCategoryDetail === "전체보기" ? "" : sellerCategoryDetail];
}
function marketCategoryNavigator() {
  const base = catalogBaseProducts();
  const selected = selectedMarketCategoryPath();
  ...
  if (depth >= 1) {
    ...
    const chipPath = depth === 4 ? selected.slice(0, 3) : selected.slice(0, depth);
    const options = categoryLevelOptions(chipLevel, chipPath);
    const countFor = option => { const path = [...chipPath]; path[chipLevel - 1] = option; return base.filter(product => productMatchesCategory(product, path)).length; };
    const crumbs = selected.filter(Boolean).map((name, index) => `${index ? `<i aria-hidden="true">›</i>` : ""}<button type="button" class="${index === depth - 1 ? "current" : ""}" data-action="category-crumb" data-level="${index + 1}">${escapeHtml(name)}</button>`).join("");
    const levelName = ["대분류", "중분류", "소분류", "세분류"][chipLevel - 1];
    const chips = options.map(option => { const count = countFor(option); const active = selected[chipLevel - 1] === option; return `<button type="button" class="category-chip ${active ? "active" : ""} ${count ? "" : "empty"}" data-action="category-chip" data-level="${chipLevel}" data-value="${escapeHtml(option)}"><b>${escapeHtml(option)}</b><em>${count}</em></button>`; }).join("");
    sub = `<div class="cat-sub"><div class="cat-sub-head"><nav class="category-crumbs" aria-label="카테고리 경로">${crumbs}</nav><button type="button" class="cat-clear" data-action="clear-market-filter" data-filter="category" aria-label="카테고리 선택 해제"><i aria-hidden="true">×</i> <span class="cat-clear-word">카테고리 </span>해제</button></div><div class="category-chip-row" aria-label="${levelName}">${chips || `<span class="category-chip-empty">가장 아래 단계예요.</span>`}</div></div>`;
  }
  return `<div class="category-nav">
    <div class="category-nav-head"><span class="category-nav-label">카테고리 <small>네이버쇼핑 기준</small></span><div class="category-search-box"><input type="text" class="category-search-input" data-category-search placeholder="카테고리 검색 (예: 꿀, 김치)" autocomplete="off"><div class="category-search-results" data-category-search-results hidden></div></div></div>
    ${rail}${sub}
  </div>`;
}
/* 지금 걸려 있는 조건을 칩으로 보여주고, × 로 하나씩 풀 수 있게 한다. */
function marketActiveFilters() {
  const chips = [];
  const path = selectedMarketCategoryPath().filter(Boolean);
  if (path.length) chips.push(["category", `카테고리 · ${path.join(" › ")}`]);
  if (sellerShippingFilter !== "all") chips.push(["shipping", sellerShippingFilter === "overseas" ? "해외소싱" : "국내배송"]);
  if (sellerCountry !== "전체 국가") chips.push(["country", `나라 · ${sellerCountry}`]);
  if (sellerProductSearch.trim()) chips.push(["search", `검색어 · ${sellerProductSearch.trim()}`]);
  if (!chips.length) return "";
  return `<div class="market-active-filters"><span class="market-active-label">선택한 조건</span>${chips.map(([key, label]) => `<button type="button" class="filter-chip" data-action="clear-market-filter" data-filter="${key}" aria-label="${escapeHtml(label)} 조건 해제"><b>${escapeHtml(label)}</b><i aria-hidden="true">×</i></button>`).join("")}${chips.length > 1 ? `<button type="button" class="filter-chip-reset" data-action="reset-market-filter">전체 해제</button>` : ""}</div>`;
}
function marketViewToggle() {
  return `<div class="market-view-toggle" role="group" aria-label="상품 보기 방식"><button type="button" class="${marketViewMode === "grid" ? "active" : ""}" data-action="set-market-view" data-view="grid" aria-label="바둑판 보기"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="3" width="8" height="8" rx="1.5"/><rect x="13" y="3" width="8" height="8" rx="1.5"/><rect x="3" y="13" width="8" height="8" rx="1.5"/><rect x="13" y="13" width="8" height="8" rx="1.5"/></svg></button><button type="button" class="${marketViewMode === "list" ? "active" : ""}" data-action="set-market-view" data-view="list" aria-label="목록 보기"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="4" width="18" height="4" rx="1"/><rect x="3" y="10" width="18" height="4" rx="1"/><rect x="3" y="16" width="18" height="4" rx="1"/></svg></button></div>`;
}
function marketProductListing(products, empty) {
  return marketViewMode === "list"
...
function marketToolbar() {
  ...
  return `<div class="market-toolbar market-toolbar-shop">
    <label class="catalog-search"><span aria-hidden="true">⌕</span><input id="sellerCatalogSearch" value="${escapeHtml(sellerProductSearch)}" placeholder="상품명·원산지·카테고리 검색" autocomplete="off" enterkeyhint="search">${sellerProductSearch ? `<button type="button" class="search-clear" data-action="clear-market-filter" data-filter="search" aria-label="검색어 지우기">×</button>` : ""}</label>
    ${marketCategoryNavigator()}
    <div class="market-filter-row">
      <div class="shipping-tabs" role="group" aria-label="배송 구분"><button type="button" class="${sellerShippingFilter === "all" ? "active" : ""}" data-action="filter-shipping" data-filter="all">전체</button><button type="button" class="${sellerShippingFilter === "domestic" ? "active" : ""}" data-action="filter-shipping" data-filter="domestic">국내배송</button><button type="button" class="${sellerShippingFilter === "overseas" ? "active" : ""}" data-action="filter-shipping" data-filter="overseas">해외소싱</button></div>
      <label class="market-select"><span>나라</span><select id="marketCountrySelect">${countries.map(country => `<option ${sellerCountry === country ? "selected" : ""}>${escapeHtml(country)}</option>`).join("")}</select></label>
      <label class="market-select"><span>정렬</span><select id="marketSortSelect"><option value="recommend" ${sellerCatalogSort === "recommend" ? "selected" : ""}>추천순</option><option value="margin" ${sellerCatalogSort === "margin" ? "selected" : ""}>마진 높은순</option><option value="supply" ${sellerCatalogSort === "supply" ? "selected" : ""}>공급가 낮은순</option></select></label>
    </div>
    ${marketActiveFilters()}
    <div class="market-result-row"><span>검색 결과 <b>${sellerCatalogProducts().length}</b>개</span>${marketViewToggle()}</div>
  </div>`;
}
...
function brandProfiles() {
  ...
  state.products.filter(product => product.status === "판매중" && product.visibility !== "비노출" && connected.has(product.supplierLoginId)).forEach(product => {
    ...
    brand.products.push(product); brand.categories.add(product.category); brand.countries.add(product.originCountry || "대한민국");
  });
  return [...map.values()].map(brand => ({ ...brand, cover: brand.products[0], tagline: BRAND_TAGLINES[brand.name] || `${brand.supplier} 공급 브랜드`, categories: [...brand.categories], countries: [...brand.countries], overseas: brand.products.some(product => product.shippingType === "overseas") }));
}
function brandLogo(name) { return `<span class="brand-logo" aria-hidden="true">${escapeHtml(String(name).replace(/^(제주|서해)\s*/, "").slice(0, 1))}</span>`; }
function brandBannerCard(brand) {
  return `<button type="button" class="brand-banner-card" data-action="select-brand" data-brand="${escapeHtml(brand.name)}" data-brand-search="${escapeHtml(`${brand.name} ${brand.supplier} ${brand.categories.join(" ")} ${brand.products.map(product => product.name).join(" ")}`.toLowerCase())}" data-brand-category="${escapeHtml(brand.categories.join("|"))}">
    <span class="brand-banner-image">${productPhoto(brand.cover, "brand-banner-photo")}<em>${brand.overseas ? `해외 · ${escapeHtml(brand.countries.join("·"))}` : "국내 브랜드"}</em></span>
    <span class="brand-banner-body">${brandLogo(brand.name)}<span><b>${escapeHtml(brand.name)}</b><small>${escapeHtml(brand.tagline)}</small><i>${escapeHtml(brand.categories.join(" · "))} · 상품 ${brand.products.length}개</i></span><strong>브랜드 상품 보기 →</strong></span>
  </button>`;
}
function filterBrandBanners() {
  const panel = document.querySelector(".brand-directory-panel");
  if (!panel) return;
  const query = (panel.querySelector("[data-brand-search-input]")?.value || "").trim().toLowerCase();
  const tab = panel.querySelector("[data-brand-tab].active")?.dataset.brandTab || "";
  let visible = 0;
  panel.querySelectorAll(".brand-banner-card").forEach(card => { const show = (!query || card.dataset.brandSearch.includes(query)) && (!tab || card.dataset.brandCategory.split("|").includes(tab)); card.hidden = !show; if (show) visible += 1; });
  const empty = panel.querySelector(".brand-empty"); if (empty) empty.hidden = visible > 0;
}
function brandDirectoryTemplate() {
  const brands = brandProfiles();
  const categories = [...new Set(brands.flatMap(brand => brand.categories))];
  return `<div class="market-intro brand-intro"><div><b>브랜드로 상품 찾기</b><span>브랜드 배너를 누르면 그 브랜드 상품만 모아서 보여 드려요.</span></div><button type="button" class="secondary-button" data-action="close-brand-directory">카테고리로 찾기 →</button></div>
    <section class="panel brand-directory-panel">
      <div class="brand-directory-tools"><label class="brand-search"><span aria-hidden="true">⌕</span><input type="text" data-brand-search-input placeholder="브랜드명·상품명으로 찾기 (예: 사과, 마누카)" autocomplete="off"></label>
      <div class="brand-category-tabs" role="tablist"><button type="button" class="active" data-brand-tab="">전체 ${brands.length}</button>${categories.map(category => `<button type="button" data-brand-tab="${escapeHtml(category)}">${escapeHtml(category)} ${brands.filter(brand => brand.categories.includes(category)).length}</button>`).join("")}</div></div>
      <div class="brand-banner-grid">${brands.length ? brands.map(brandBannerCard).join("") : ""}</div>
      <div class="empty brand-empty" ${brands.length ? "hidden" : ""}><b>찾는 브랜드가 없어요.</b><span>다른 이름으로 검색해 보세요.</span></div>
    </section>`;
}
...
function sellerBrandSourcingTemplate() {
  ...
  if (!brand) { sellerBrand = "전체 브랜드"; return brandDirectoryTemplate(); }
  const empty = `<div class="empty catalog-empty"><b>이 브랜드에 판매 가능한 상품이 없어요.</b></div>`;
  return `${selectedBrandHero(brand)}<div id="marketProductGrid" class="panel seller-market-panel brand-products-panel"><div class="market-result-row"><span><b>${escapeHtml(brand.name)}</b> 상품 <b>${brand.products.length}</b>개</span>${marketViewToggle()}</div>${marketProductListing(brand.products, empty)}</div>`;
}
...
function sellerMarketplaceTemplate() {
  ...
  const empty = `<div class="empty catalog-empty"><b>조건에 맞는 상품이 없어요.</b><span>위의 ‘선택한 조건’에서 × 를 눌러 조건을 하나씩 풀어 보세요.</span><button type="button" class="secondary-button" data-action="reset-market-filter">조건 전체 해제</button></div>`;
  return `<div class="market-intro"><div><b>카테고리로 상품 찾기</b><span>네이버쇼핑과 같은 카테고리로 찾고, 마음에 들면 PICK하세요.</span></div><button type="button" class="secondary-button" data-action="open-brand-directory">브랜드로 찾기 →</button></div><div id="marketProductGrid" class="panel seller-market-panel">${marketToolbar()}${marketProductListing(products, empty)}</div>`;
}
...
function sellerItemStage(item) {
  if (!item) return "none";
  if (item.approvalStatus === "승인거절") return "rejected";
  if (item.approvalStatus !== "승인완료") return "pending";
  if (liveChannelIds(item).length) return "live";
...
function sellerLiveProducts() { return currentSellerProducts().filter(item => item.approvalStatus === "승인완료" && liveChannelIds(item).length); }
...
function sellerItemThumb(item, product = productOf(item?.productId), className = "") {
  return `<span class="thumb-wrap">${sellerItemPhoto(item, product, className)}${item?.thumbBadge ? `<em class="thumb-badge">${escapeHtml(item.thumbBadge)}</em>` : ""}</span>`;
}
function productCustomizeChecklist(item, product) {
  return [
    ["상품명", Boolean(item.customTitle && item.customTitle !== product?.name)],
    ["대표 사진", Boolean(item.customThumbnail || item.thumbBadge)],
    ["상세페이지", Boolean(item.detailEdited)],
    ["판매가", Boolean(item.pricesEdited)]
  ];
}
...
function sellerApprovedProductsTemplate() {
  const items = sellerMasterProducts();
  const waiting = items.filter(item => !liveChannelIds(item).length);
  const live = items.filter(item => liveChannelIds(item).length);
  const shown = masterStatusFilter === "waiting" ? waiting : masterStatusFilter === "live" ? live : [...waiting, ...live];
  return `${sectionHero("마스터 상품", "내 상품으로 등록된 목록이에요. ‘상품 전송’을 누르면 쿠팡·스마트스토어에 옵션까지 그대로 한 번에 등록됩니다.")}
    ...
    <div class="panel"><div class="panel-head master-list-head"><div><h3>내 마스터 상품 <span class="chip">${items.length}개</span></h3><p><b class="legend-wait">전송 대기</b>는 아직 쇼핑몰에 안 올린 상품, <b class="legend-live">판매중</b>은 쇼핑몰 전송을 마친 상품이에요.</p></div><div class="view-toggle" role="group" aria-label="보기 방식"><button type="button" class="${masterProductView === "grid" ? "active" : ""}" data-action="master-view" data-view="grid" aria-pressed="${masterProductView === "grid"}"><span aria-hidden="true">▦</span> 바둑판</button><button type="button" class="${masterProductView === "list" ? "active" : ""}" data-action="master-view" data-view="list" aria-pressed="${masterProductView === "list"}"><span aria-hidden="true">☰</span> 가로형</button></div></div>
    <div class="master-status-filter" role="tablist" aria-label="상태로 보기">${[["all", "전체", items.length], ["waiting", "전송 대기", waiting.length], ["live", "판매중", live.length]].map(([key, label, count]) => `<button type="button" role="tab" class="${masterStatusFilter === key ? "active" : ""} ${key}" data-action="master-status-filter" data-filter="${key}" aria-selected="${masterStatusFilter === key}">${label} <b>${count}</b></button>`).join("")}</div>
    ${items.length ? (shown.length ? `<div class="my-product-list view-${masterProductView}">${shown.map(item => myProductCard(item, "master")).join("")}</div>` : `<div class="empty mapping-empty"><b>${masterStatusFilter === "waiting" ? "전송을 기다리는 상품이 없어요." : "아직 판매중인 상품이 없어요."}</b></div>`) : `<div class="empty mapping-empty"><b>아직 마스터 상품이 없어요.</b><span>‘승인 완료’에서 상품을 꾸민 뒤 등록하면 여기에 들어와요.</span><button type="button" class="primary-button" data-action="go-menu" data-menu="승인 완료">승인 완료 보기</button></div>`}
    </div>`;
}
...
function sellerOnSaleProductsTemplate() {
  const liveItems = sellerLiveProducts();
  ...
  const filtered = liveItems.filter(item => {
    const product = productOf(item.productId);
    const matchesQuery = !query || [sellerProductTitle(item, product), item.productId, item.id, ...productOptions(product).map(option => option.name)].some(value => String(value || "").toLowerCase().includes(query));
    ...
    const matchesCategory = onSaleCategoryFilter === "전체" || product?.category === onSaleCategoryFilter;
    return matchesQuery && matchesFrom && matchesTo && matchesCategory;
  });
  ...
  return `${sectionHero("판매중 상품", "쇼핑몰에 올라가 실제로 판매 중인 상품이에요. 상품을 누르면 쇼핑몰별 옵션·가격·코드를 볼 수 있어요.")}${productStageTabs("판매중 상품")}
    ...
    <div class="onsale-top"><label class="onsale-search"><span aria-hidden="true">⌕</span><input id="onSaleSearchInput" value="${escapeHtml(onSaleSearch)}" placeholder="상품명·옵션·코드 검색" autocomplete="off"></label>
      <details class="onsale-more" ${filtersActive ? "open" : ""}><summary>필터${filtersActive ? ` <i aria-label="필터 적용됨"></i>` : ""}</summary>
        <div class="onsale-filter-bar">
          <label class="onsale-category-field"><span>카테고리</span><select id="onSaleCategorySelect">${onSaleCategories.map(category => `<option value="${escapeHtml(category)}" ${onSaleCategoryFilter === category ? "selected" : ""}>${escapeHtml(category)}</option>`).join("")}</select></label>
          <label class="onsale-date-field"><span>등록일 부터</span><input type="date" id="onSaleDateFromInput" value="${escapeHtml(onSaleDateFrom)}"></label>
          <label class="onsale-date-field"><span>까지</span><input type="date" id="onSaleDateToInput" value="${escapeHtml(onSaleDateTo)}"></label>
    ...
    <div class="onsale-count">판매중 <b>${filtered.length}</b>개</div>
    ${paged.length ? `<div class="onsale-list">${paged.map(onSaleCard).join("")}</div>${onSalePagination(totalPages)}` : `<div class="empty mapping-empty"><b>${liveItems.length ? "조건에 맞는 판매중 상품이 없어요." : "아직 판매중인 상품이 없어요."}</b><span>${liveItems.length ? "검색어나 필터를 바꿔 보세요." : "마스터 상품에서 ‘상품 전송’을 눌러 쇼핑몰에 올려 보세요."}</span>${liveItems.length ? `<button type="button" class="secondary-button" data-action="reset-onsale-filter">필터 초기화</button>` : `<button type="button" class="primary-button" data-action="go-menu" data-menu="마스터 상품">마스터 상품 보기</button>`}</div>`}
  </div>`;
}
...
function onSaleChannelBlock(item, channel, product) {
  ...
  const priceTable = options.length
    ? `<div class="onsale-options" role="table" aria-label="${escapeHtml(channel.name)} 옵션"><div class="onsale-options-head" role="row"><span>옵션 · 쇼핑몰 코드</span><span>판매가</span><span>마진</span></div>${options.map(option => { const source = productOptionOf(product, option.optionId); return `<div role="row"><span><b>${escapeHtml(option.name)}</b><code>${escapeHtml(option.code)}</code></span><span>${money(option.salePrice)}</span><span class="profit-text">${margin(source?.supply ?? product?.supply ?? 0, option.salePrice)}%</span></div>`; }).join("")}</div>`
    : `<div class="onsale-single"><span>판매가 <b>${money(detail.salePrice || item.salePrice)}</b></span><span>마진 <b class="profit-text">${margin(product?.supply || 0, detail.salePrice || item.salePrice)}%</b></span><span>코드 <code>${escapeHtml(channelProductCode(item, channel.id))}</code></span></div>`;
...
const CHANNEL_CODE_PREFIX = { coupang: "CP", smartstore: "NS", kakao: "KK", cafe24: "C24" };
function channelProductCode(item, channelId) { return `${CHANNEL_CODE_PREFIX[channelId] || "EX"}-${String(item.id).replace(/^SP-/, "")}`; }
/* 옵션 상품은 쇼핑몰 옵션마다 코드가 붙는다. 예) 쿠팡 CP-1003-2 = SP-1003의 2번째 옵션 */
function channelOptionCode(item, channelId, index) { return `${channelProductCode(item, channelId)}-${index}`; }
function sellerItemCategoryText(item, product = productOf(item?.productId)) {
...
/* 채널에 실제로 올라간 옵션 목록 (예전 데이터는 현재 옵션 설정으로 보여 준다) */
function channelListingOptions(item, channelId, product = productOf(item?.productId)) {
  const saved = item.channelDetails?.[channelId]?.options;
...
/* 옵션 코드(CP-1004-2)면 기본 코드(CP-1004)와 옵션 순번(2)으로 나눈다 */
function parseChannelOptionCode(code) { const match = String(code || "").match(/^(.+-\d+)-(\d+)$/); return match ? { baseCode: match[1], index: Number(match[2]) } : null; }
function resolveMappingForCode(code) {
  const direct = autoLinkedSellerProduct(code);
  const optionCode = direct ? null : parseChannelOptionCode(code);
  const linked = direct || (optionCode ? autoLinkedSellerProduct(optionCode.baseCode) : null);
  if (linked && productOf(linked.productId)) {
    const product = productOf(linked.productId);
    const option = optionCode ? productOptions(product)[optionCode.index - 1] || null : null;
...
function applyMappingToOrder(order, product, type, option = null) {
  ...
  order.carrier = product.carrier || order.carrier || "한진택배";
  if (option) { order.optionId = option.id; order.optionName = option.name; }
  else if (!hasOptions(product)) { delete order.optionId; delete order.optionName; }
  order.supplyTotal = orderUnitSupply(order, product) * Number(order.qty || 1);
}
function orderOption(order, product = orderSourceProduct(order)) { return order?.optionId ? productOptionOf(product, order.optionId) : null; }
function orderUnitSupply(order, product = orderSourceProduct(order)) { const option = orderOption(order, product); return Number(option ? option.supply : product?.supply || 0); }
function orderOptionTag(order) { return order?.optionName ? `<em class="order-option-tag">옵션 · ${escapeHtml(order.optionName)}</em>` : ""; }
...
function supplierPickApprovalPanel() {
  ...
  return `<section class="panel pick-approval-panel">
    <div class="pick-approval-head"><div><span>PICK 승인 요청</span><h3>위탁셀러가 판매하고 싶은 상품이에요</h3><p>승인하면 위탁셀러의 ‘승인 완료’ 상품이 되고, 위탁셀러가 꾸민 뒤 쇼핑몰에 옵션 그대로 등록할 수 있어요.</p></div>
...
function dashboardWidgetPickerModal() {
  ...
  const labels = SELLER_WIDGET_LABELS;
...
function renderSeller() {
  ...
  const countStatus = (...statuses) => sellerOrders.filter(order => statuses.includes(order.status) && orderMappingStatus(order) === "mapped" && orderPaymentStatus(order) !== "pending").length;
  ...
  const readyItems = sellerReadyProducts();
  const masterReady = sellerMasterProducts().filter(item => !liveChannelIds(item).length);
  ...
  document.getElementById("sellerView").innerHTML = `
    ${editMode ? `<div class="dashboard-edit-toolbar"><div><span>홈 화면 편집 중</span><b>칸을 끌어서 순서를 바꾸고 × 버튼으로 숨길 수 있어요.</b></div><div><button type="button" class="secondary-button" data-action="add-dashboard-widget">＋ 칸 추가</button><button type="button" class="secondary-button" data-action="reset-dashboard-layout">기본 배치</button><button type="button" class="secondary-button" data-action="cancel-dashboard-edit">취소</button><button type="button" class="primary-button" data-action="save-dashboard-layout">저장</button></div></div>` : ""}
    <div class="dashboard-widget-canvas home-canvas">${layout.map(id => dashboardWidgetFrame(id, SELLER_WIDGET_LABELS[id], widgets[id])).join("")}${editMode ? `<button type="button" class="dashboard-add-widget" data-action="add-dashboard-widget"><span>＋</span><b>칸 추가</b><small>숨긴 칸을 다시 넣을 수 있어요.</small></button>` : ""}</div>
    ${editMode ? "" : `<div class="home-edit-link"><button type="button" class="text-button" data-action="toggle-edit-mode">⚙ 홈 화면 편집</button></div>`}`;
}
...
function catalogPriceRows(p) {
  ...
  const priceRow = `<div><dt>지정판매가</dt><dd class="${freePriced ? "" : "profit-text"}">${freePriced ? "자율" : productPriceLabel(p, "recommended")}</dd></div>`;
  const marginRow = `<div><dt>마진율</dt><dd class="${freePriced ? "margin-free" : ""}">${freePriced ? "-%" : `${margin(p.supply, p.recommended)}%`}</dd></div>`;
  return `<div><dt>공급가</dt><dd>${productPriceLabel(p)}</dd></div>${priceRow}${marginRow}`;
}
function productRowSeller(p) {
  ...
  const pickState = sellerItemStage(sellerItem);
...
function productRowSellerList(p) {
  ...
  return `<article class="market-product-list-row" data-action="product-detail" data-id="${p.id}" tabindex="0" aria-label="${escapeHtml(p.name)} 상세 보기">
    ...
    <div class="list-row-body">
      <span class="list-row-badges"><em class="shipping-badge ${p.shippingType === "overseas" ? "overseas" : "domestic"}">${p.shippingType === "overseas" ? `해외직구 · ${escapeHtml(p.originCountry)}` : "국내배송"}</em>${hasOptions(p) ? `<em class="option-count-badge">옵션 ${productOptions(p).length}개</em>` : ""}<small>발주마감 ${escapeHtml(p.cutoff || "10:00")}</small></span>
...
function supplierPerformanceTemplate() {
  ...
  const rows = products.map(product => {
    ...
    const grossSettlement = productOrders.filter(order => !state.refunds.some(refund => refund.orderId === order.id && refund.status === "환불완료")).reduce((sum, order) => sum + orderUnitSupply(order, product) * order.qty, 0);
...
function stringSeed(text) { return [...String(text)].reduce((sum, char) => (Math.imul(sum, 31) + char.charCodeAt(0)) >>> 0, 17); }
function roundPrice(value) { return Math.max(100, Math.round(value / 100) * 100); }
/* 셀러 판매가 목록 + 날짜별 이력 */
function marketListingsFor(product, days = 90) {
  const recommended = Number(product.recommended || Math.round(Number(product.supply || 0) * 1.6));
  const random = seededRandom(stringSeed(product.id));
  const listings = [];
  state.sellerProducts.filter(item => item.productId === product.id && item.approvalStatus === "승인완료").forEach(item => {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const history = [];
    let level = roundPrice(base * (1 + random() * 0.14));
    let promo = 0;
    let changedDaysAgo = days;
    for (let day = 0; day < days; day += 1) {
      const remaining = days - 1 - day;
      if (remaining < 3) level = base;
      else if (random() < 0.05) { level = roundPrice(base * (0.9 + random() * 0.22)); changedDaysAgo = remaining; }
      if (!promo && random() < 0.018) promo = 2 + Math.floor(random() * 3);
      history.push(promo ? roundPrice(level * 0.8) : level);
      if (promo) promo -= 1;
    }
    listings.push({ seller: MARKET_SELLER_NAMES[(stringSeed(product.id) + index) % MARKET_SELLER_NAMES.length], channelId: random() < 0.55 ? "smartstore" : "coupang", price: history[days - 1], real: false, changedDaysAgo: Math.min(changedDaysAgo, days - 1), history });
  }
  return { recommended, listings };
}
function priceGraphModel(product, days) {
  const { recommended, listings } = marketListingsFor(product, 90);
  ...
  const allMax = Math.max(...points.map(point => point.max)), allMin = Math.min(...points.map(point => point.min));
  const avg = roundPrice(points.reduce((sum, point) => sum + point.avg, 0) / points.length);
  return { product, recommended, listings: listings.sort((a, b) => a.price - b.price), points, allMax, allMin, avg, current: points[points.length - 1] };
}
function priceGraphSvg(model) {
  const mobile = window.innerWidth <= 720;
  const W = mobile ? 360 : 760, H = mobile ? 300 : 280, padL = mobile ? 6 : 10, padR = mobile ? 6 : 10, padT = 34, padB = 30;
  const values = [model.allMax, model.allMin, model.recommended];
  const hi = Math.max(...values), lo = Math.min(...values);
  const span = Math.max(1, hi - lo);
  const yMax = hi + span * 0.08, yMin = lo - span * 0.08;
  const x = index => padL + (W - padL - padR) * (model.points.length === 1 ? 0 : index / (model.points.length - 1));
  const y = value => padT + (H - padT - padB) * (1 - (value - yMin) / (yMax - yMin));
  const step = key => model.points.map((point, index) => index === 0 ? `M${x(0).toFixed(1)},${y(point[key]).toFixed(1)}` : `H${x(index).toFixed(1)}V${y(point[key]).toFixed(1)}`).join("");
  const ticks = mobile ? 3 : 5;
  const tickIdx = Array.from({ length: ticks }, (_, i) => Math.round((model.points.length - 1) * i / (ticks - 1)));
  const ref = (value, color, label, above) => `<line x1="${padL}" x2="${W - padR}" y1="${y(value).toFixed(1)}" y2="${y(value).toFixed(1)}" stroke="${color}" stroke-width="1.5" stroke-dasharray="2 5" opacity=".55"/><text x="${W - padR}" y="${(y(value) + (above ? -8 : 17)).toFixed(1)}" text-anchor="end" class="pg-ref-label">${label}</text>`;
  return `<svg class="pg-svg" viewBox="0 0 ${W} ${H}" role="img" aria-label="${escapeHtml(model.product.name)} ${model.points.length}일 판매가 그래프: 최고 ${money(model.allMax)}, 평균 ${money(model.avg)}, 최저 ${money(model.allMin)}" tabindex="0" data-w="${W}" data-padl="${padL}" data-padr="${padR}">
    ${tickIdx.map(index => `<line x1="${x(index).toFixed(1)}" x2="${x(index).toFixed(1)}" y1="${padT - 6}" y2="${H - padB}" class="pg-grid"/>`).join("")}
    <line x1="${padL}" x2="${W - padR}" y1="${y(model.recommended).toFixed(1)}" y2="${y(model.recommended).toFixed(1)}" class="pg-rec-line"/><text x="${padL + 2}" y="${(y(model.recommended) - 7).toFixed(1)}" class="pg-rec-label">권장 소비자가 ${money(model.recommended)}</text>
    ${ref(model.allMax, PRICE_GRAPH_COLORS.max, `▲ 기간 최고가 ${money(model.allMax)}`, true)}
    ${ref(model.allMin, PRICE_GRAPH_COLORS.min, `▼ 기간 최저가 ${money(model.allMin)}`, false)}
    <path d="${step("max")}" fill="none" stroke="${PRICE_GRAPH_COLORS.max}" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"/>
    <path d="${step("min")}" fill="none" stroke="${PRICE_GRAPH_COLORS.min}" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"/>
    <path d="${step("avg")}" fill="none" stroke="${PRICE_GRAPH_COLORS.avg}" stroke-width="2" stroke-linejoin="round" stroke-linecap="round" stroke-dasharray="6 4"/>
    ${tickIdx.map((index, i) => `<text x="${x(index).toFixed(1)}" y="${H - 8}" text-anchor="${i === 0 ? "start" : i === ticks - 1 ? "end" : "middle"}" class="pg-axis">${model.points[index].date}</text>`).join("")}
    <g class="pg-cross" style="display:none"><line class="pg-cross-line" y1="${padT - 6}" y2="${H - padB}"/>${["max", "avg", "min"].map(key => `<circle class="pg-dot" data-key="${key}" r="4.5" fill="${PRICE_GRAPH_COLORS[key]}"/>`).join("")}</g>
...
function supplierPriceGraphTemplate(products) {
  ...
  const mobile = window.innerWidth <= 720;
  const W = mobile ? 360 : 760, H = mobile ? 300 : 280;
  const values = [model.allMax, model.allMin, model.recommended]; const hi = Math.max(...values), lo = Math.min(...values), span = Math.max(1, hi - lo);
  priceGraphData = { points: model.points, W, H, padL: mobile ? 6 : 10, padR: mobile ? 6 : 10, padT: 34, padB: 30, yMax: hi + span * 0.08, yMin: lo - span * 0.08 };
  const diff = model.current.avg - model.recommended;
  const pct = Math.round(diff / model.recommended * 100);
  const buckets = 5;
  const bMin = Math.min(...model.listings.map(listing => listing.price)), bMax = Math.max(...model.listings.map(listing => listing.price));
  const width = Math.max(1, (bMax - bMin) / buckets);
  const dist = Array.from({ length: buckets }, (_, i) => { const from = bMin + width * i, to = i === buckets - 1 ? bMax : bMin + width * (i + 1); return { from: roundPrice(from), to: roundPrice(to), count: model.listings.filter(listing => listing.price >= from && (i === buckets - 1 ? listing.price <= to : listing.price < to)).length }; });
  const distMax = Math.max(...dist.map(bucket => bucket.count), 1);
  const weekly = model.points.filter((_, index) => (model.points.length - 1 - index) % 7 === 0);
...
function updatePriceGraphCursor(svg, index) {
  const data = priceGraphData; if (!data || !svg) return;
  const n = data.points.length; index = Math.max(0, Math.min(n - 1, index));
  svg.dataset.index = index;
  const x = data.padL + (data.W - data.padL - data.padR) * (n === 1 ? 0 : index / (n - 1));
  const y = value => data.padT + (data.H - data.padT - data.padB) * (1 - (value - data.yMin) / (data.yMax - data.yMin));
  const point = data.points[index];
  const cross = svg.querySelector(".pg-cross"); cross.style.display = "";
  const line = cross.querySelector(".pg-cross-line"); line.setAttribute("x1", x); line.setAttribute("x2", x);
  cross.querySelectorAll(".pg-dot").forEach(dot => { dot.setAttribute("cx", x); dot.setAttribute("cy", y(point[dot.dataset.key])); });
  const tip = svg.parentElement.querySelector(".pg-tip");
  tip.hidden = false;
  tip.replaceChildren();
  const head = document.createElement("b"); head.textContent = `${point.date} · 셀러 ${point.count}곳`; tip.appendChild(head);
  [["max", "▲ 최고가"], ["avg", "평균가"], ["min", "▼ 최저가"]].forEach(([key, label]) => { const row = document.createElement("span"); const swatch = document.createElement("i"); swatch.style.background = PRICE_GRAPH_COLORS[key]; const value = document.createElement("strong"); value.textContent = money(point[key]); const name = document.createElement("small"); name.textContent = label; row.append(swatch, value, name); tip.appendChild(row); });
  const box = svg.getBoundingClientRect();
  const px = x / data.W * box.width;
  tip.style.left = `${Math.min(Math.max(px, 70), box.width - 70)}px`;
}
document.addEventListener("pointermove", event => {
  const svg = event.target.closest?.(".pg-svg"); if (!svg || !priceGraphData) return;
  const box = svg.getBoundingClientRect(); const data = priceGraphData;
  const ratio = ((event.clientX - box.left) / box.width * data.W - data.padL) / (data.W - data.padL - data.padR);
  updatePriceGraphCursor(svg, Math.round(ratio * (data.points.length - 1)));
});
document.addEventListener("pointerleave", event => { if (event.target?.classList?.contains("pg-svg")) { event.target.querySelector(".pg-cross").style.display = "none"; event.target.parentElement.querySelector(".pg-tip").hidden = true; } }, true);
document.addEventListener("keydown", event => {
  const svg = event.target.closest?.(".pg-svg"); if (!svg || !priceGraphData || !["ArrowLeft", "ArrowRight", "Home", "End"].includes(event.key)) return;
  event.preventDefault();
  const current = Number(svg.dataset.index ?? priceGraphData.points.length - 1);
  updatePriceGraphCursor(svg, event.key === "Home" ? 0 : event.key === "End" ? priceGraphData.points.length - 1 : current + (event.key === "ArrowLeft" ? -1 : 1));
});
document.addEventListener("focusin", event => { if (event.target?.classList?.contains("pg-svg") && priceGraphData) updatePriceGraphCursor(event.target, priceGraphData.points.length - 1); });
...
function productDetailModal(id) {
  ...
  const pickLabel = PICK_STAGE_LABELS[pickState];
  openModal(`<div class="product-detail-page"><div class="product-breadcrumb"><button type="button" data-action="open-catalog">공급 상품몰</button>${productCategoryPath(p).map((name, index, all) => `<span>›</span><button type="button" data-action="category-jump" data-path="${escapeHtml(all.slice(0, index + 1).join("|"))}">${escapeHtml(name)}</button>`).join("")}</div><div class="product-detail">
    <div class="product-detail-visual">${productPhoto(p, "detail-photo")}<small>${escapeHtml(p.categorySub || p.category)}</small><span class="detail-shipping-badge ${overseas ? "overseas" : ""}">${overseas ? `해외직구 · ${escapeHtml(p.originCountry)}` : "국내배송"}</span></div>
...
/* PICK·꾸미기 폼에서 옵션 가격을 읽어 검증한다. 문제가 있으면 message를 돌려준다. */
function readOptionPrices(form, product, namePrefix = "optionPrice") {
  const options = productOptions(product);
  const formData = new FormData(form);
  const enabledIds = formData.getAll("optionEnabled");
  if (!enabledIds.length) return { message: "판매할 옵션을 하나 이상 골라 주세요." };
  const optionPrices = {}, optionEnabled = {};
  for (const option of options) {
    const price = Number(formData.get(`${namePrefix}__${option.id}`) || option.recommended || option.supply);
    const enabled = enabledIds.includes(option.id);
    if (enabled && price < Number(option.supply)) return { message: `‘${option.name}’ 판매가가 공급가(${money(option.supply)})보다 낮아요.` };
    optionPrices[option.id] = price;
    optionEnabled[option.id] = enabled;
  }
  const salePrice = Math.min(...options.filter(option => optionEnabled[option.id]).map(option => optionPrices[option.id]));
  return { optionPrices, optionEnabled, salePrice };
}
function importModal(id) {
  const p = productOf(id);
  if (!p) return;
  const auto = supplierAutoApprove(p.supplierLoginId);
  const options = productOptions(p);
  openModal(`<div class="pick-sheet">
    <div class="pick-sheet-head">${productPhoto(p, "pick-sheet-photo")}<div><small>${escapeHtml(productBrand(p))} · ${escapeHtml(p.id)}</small><h2>${escapeHtml(p.name)}</h2><span class="pick-sheet-tags"><em class="shipping-badge ${p.shippingType === "overseas" ? "overseas" : "domestic"}">${p.shippingType === "overseas" ? `해외직구 · ${escapeHtml(p.originCountry || "")}` : "국내배송"}</em>${options.length ? `<em class="option-count-badge">옵션 ${options.length}개</em>` : ""}</span></div></div>
    <form id="importForm" class="pick-form" data-id="${p.id}">
      <div class="pick-section-title"><b>${options.length ? "판매할 옵션과 내 판매가" : "내 판매가"}</b><small>${options.length ? "체크한 옵션만 쇼핑몰에 똑같이 올라가요" : "나중에 ‘승인 완료’에서 바꿀 수 있어요"}</small></div>
      ${options.length ? optionPriceRows(p) : singlePriceRow(p, p.recommended)}
      <div class="pick-approval-note ${auto ? "auto" : ""}">${auto ? "<b>⚡ 자동 승인 공급사예요</b><span>PICK하면 바로 승인돼요. ‘승인 완료’에서 상품명·사진을 꾸미고 등록하세요.</span>" : "<b>공급사 승인 후 내 상품이 돼요</b><span>공급사가 승인하면 알림으로 알려 드려요.</span>"}</div>
      <div class="modal-actions pick-actions"><button type="button" class="secondary-button" data-close-modal>취소</button><button type="submit" class="primary-button">PICK하기</button></div>
    </form></div>`);
  document.querySelector("#modal .modal").classList.add("pick-sheet-modal");
}
function updatePriceMargin(input) {
  const scope = input.closest("form") || document;
  const target = input.dataset.marginTarget;
  const label = scope.querySelector(`[data-margin-for="${CSS.escape(target)}"]`);
  if (!label) return;
  const value = margin(input.dataset.supply, input.value);
  label.textContent = Number(input.value) < Number(input.dataset.supply) ? "공급가보다 낮아요" : `마진 ${value}%`;
  label.classList.toggle("low", Number(input.value) < Number(input.dataset.supply) || value < 10);
}

/* ===== 상품 꾸미기(스튜디오): 대표 사진 · 상품명 · 옵션 가격 · 상세페이지 블록 · 카테고리 ===== */
  ...
  openModal(`<div class="studio">
    ...
    <form id="studioForm" class="studio-form" data-id="${item.id}">
      ...
      <section id="studio-title" class="studio-section"><h3><i>2</i> 상품명</h3><p>‘지역 + 상품 + 중량’처럼 쓰면 검색에 잘 걸려요.</p>
        <div class="studio-title-field"><input id="studioTitle" name="customTitle" value="${escapeHtml(title)}" maxlength="50" required autocomplete="off"><em id="studioTitleCount">${title.length}/50</em></div>
        <div class="studio-title-words"><span>추천 단어</span>${TITLE_WORDS.map(word => `<button type="button" data-title-word="${escapeHtml(word)}">+ ${escapeHtml(word)}</button>`).join("")}<button type="button" class="studio-ghost" data-title-original="${escapeHtml(source.name)}">원래 이름</button></div>
...
/* 쇼핑몰에 올라가 있는 판매 단위 목록 (옵션 상품은 옵션마다 하나씩) — 주문 불러오기·자동 수집에 사용 */
function liveListingSources(sellerLoginId = currentAccount?.loginId || "seller") {
  return state.sellerProducts.filter(item => item.sellerLoginId === sellerLoginId && item.approvalStatus === "승인완료").flatMap(item => {
    const product = productOf(item.productId);
    if (!product) return [];
    return liveChannelIds(item).flatMap(channelId => {
      const channel = channelMeta(channelId);
      const detail = sellerChannelDetail(item, channel, product);
      const title = detail.title || sellerProductTitle(item, product);
      const options = hasOptions(product) ? channelListingOptions(item, channelId, product) : [];
      if (options.length) return options.map(option => ({ code: option.code, name: `${title} · ${option.name}`, channelId, channelName: channel.name, amount: Number(option.salePrice), overseas: productNeedsCustoms(product), item, product, optionId: option.optionId }));
      return [{ code: channelProductCode(item, channelId), name: title, channelId, channelName: channel.name, amount: Number(detail.salePrice || item.salePrice), overseas: productNeedsCustoms(product), item, product, optionId: "" }];
    });
  });
}
function externalOrderModal() {
  const sources = liveListingSources();
  const liveOptions = sources.map(source => ({ item: source.item, channelId: source.channelId }));
  const first = sources[0];
  const externalCode = `EX-${String(Date.now()).slice(-6)}`;
  openModal(`<div class="external-order-head"><span>MARKETPLACE ORDER</span><h2>쇼핑몰 주문 불러오기</h2><p>쿠팡·스마트스토어에 들어온 신규주문을 가져옵니다. 두고에서 전송한 상품은 자동으로 공급사 상품에 연결됩니다.</p></div>
    <form id="externalOrderForm" class="form-grid order-form">
      <div class="form-field full"><label>주문 들어온 상품</label><select id="externalOrderSource">${sources.map(source => `<option value="${escapeHtml(source.code)}" data-name="${escapeHtml(source.name)}" data-channel="${escapeHtml(source.channelName)}" data-amount="${source.amount}" data-overseas="${source.overseas ? "true" : ""}">[자동 연결] ${escapeHtml(source.channelName)} · ${escapeHtml(source.name)}</option>`).join("")}<option value="${externalCode}" data-name="쿠팡 판매상품 샘플" data-channel="쿠팡" data-amount="29900" data-manual="true">[연결 필요] 두고에서 전송하지 않은 쇼핑몰 상품</option></select><small class="field-hint">두고에서 전송한 상품은 매핑 없이 바로 결제 단계로 갑니다.</small></div>
      <div class="form-field full"><label>쇼핑몰 상품명 *</label><input name="externalProductName" value="${escapeHtml(first ? first.name : "쿠팡 판매상품 샘플")}" placeholder="판매채널에 등록한 상품명" required></div>
      <div class="form-field"><label>쇼핑몰 상품코드</label><input name="externalProductCode" value="${escapeHtml(first ? first.code : externalCode)}" placeholder="쿠팡 상품번호 등"></div>
...
function orderDetailModal(orderId) {
  ...
  openModal(`<div class="order-detail-head"><div><span>DOOGO ORDER</span><h2>${order.id}</h2><p>${escapeHtml(order.createdAt)} · ${channelMark(channelIdFromName(order.channel),true)} ${escapeHtml(order.channel)}</p></div>${statusChip(order.status)}</div>
    ...
    ${activeRole === "seller" && mapped ? (() => {
      ...
      const marginPct = margin(orderUnitSupply(order, product), unitSalePrice);
...
function channelApiNow() { return new Date().toLocaleString("ko-KR", { month: "numeric", day: "numeric", hour: "numeric", minute: "2-digit" }); }
function channelListing(item, channelId) { return item?.channelListings?.[channelId] || null; }
...
/* 예전 데이터(연동 기록 없이 판매중인 상품)는 처음 쓸 때 상품번호를 채워 둔다 */
function ensureChannelListing(item, channelId) {
  item.channelListings = item.channelListings || {};
  if (!item.channelListings[channelId]) item.channelListings[channelId] = { ...demoListingIds(item, channelId, sellerProductChannelStatus(item, { id: channelId }) || "판매중"), registeredAt: "이전 등록" };
  return item.channelListings[channelId];
}
...
function doogoListingFor(item, channelId) {
  const product = productOf(item.productId);
  const channel = sellerChannels(item.sellerLoginId).find(entry => entry.id === channelId);
  const detail = item.channelDetails?.[channelId] || channelListingSnapshot(item, channelId, product);
  const chosen = detail.shippingPolicy || channelDefaultPolicy(channel) || {};
...
async function channelServerCall(method, path, body) {
  ...
  if (!res.ok) throw new Error(data.error || `연동 서버 오류 (${res.status})`);
  return data;
}
/* 쇼핑몰 동작 하나를 실행한다. 데모 모드는 바로 반영되고, 서버 모드는 결과가 오면 다시 그린다. */
function runChannelOp(item, channelId, op, { stock, silent = false } = {}) {
  if (!item || !channelApiSupported(channelId)) return Promise.resolve(null);
  const channel = sellerChannels(item.sellerLoginId).find(entry => entry.id === channelId);
  const listing = op === "register" ? null : ensureChannelListing(item, channelId);
  const mode = CHANNEL_SERVER ? "server" : "demo";
  const base = { sellerLoginId: item.sellerLoginId, itemId: item.id, channelId, op, title: sellerProductTitle(item, productOf(item.productId)), mode };
  const apply = result => {
    item.channelListings = item.channelListings || {};
    if (op === "register") {
    ...
    logChannelApi({ ...base, request: channelApiRequestLine(channelId, op, item.channelListings[channelId]), externalId: item.channelListings[channelId].externalId, status: result.status, note: result.note || "", ok: true });
    return result;
  };
  if (!CHANNEL_SERVER) {
  ...
  const seller = item.sellerLoginId;
  const request = op === "register" ? channelServerCall("POST", "/api/listings", { sellerLoginId: seller, channel: channelId, listing: doogoListingFor(item, channelId) })
    : op === "remove" ? channelServerCall("DELETE", `/api/listings/${channelId}/${encodeURIComponent(listing.externalId)}`, { sellerLoginId: seller, vendorItemIds: listing.vendorItemIds || [] })
    : op === "sync" ? channelServerCall("POST", `/api/listings/${channelId}/${encodeURIComponent(listing.externalId)}/sync`, { sellerLoginId: seller, listing: doogoListingFor(item, channelId), vendorItemMap: listing.vendorItemMap || {} })
    : channelServerCall("POST", `/api/listings/${channelId}/${encodeURIComponent(listing.externalId)}/status`, { sellerLoginId: seller, action: op, stock, vendorItemIds: listing.vendorItemIds || [] });
...
/* 쇼핑몰에 아직 남아 있는 곳: 판매중이거나, 판매중지·품절로 바꿨지만 지우지 않은 곳 */
function channelsOnMarket(item) { return [...new Set([...liveChannelIds(item), ...Object.entries(item?.channelListings || {}).filter(([, listing]) => listing && !listing.deleted).map(([channelId]) => channelId)])]; }
function sellerProductDeleteModal(sellerProductId) {
  const item = state.sellerProducts.find(entry => entry.id === sellerProductId && entry.sellerLoginId === currentAccount.loginId);
  if (!item) return;
  const product = productOf(item.productId);
  const live = channelsOnMarket(item);
  const rows = live.map(channelId => {
    const rule = CHANNEL_RULES[channelId];
    const listing = channelApiSupported(channelId) ? ensureChannelListing(item, channelId) : null;
    return `<div class="delete-channel-row">${channelMark(channelId, true)}<span><b>${escapeHtml(channelMeta(channelId).name)}</b><small>${escapeHtml(rule ? rule.removeNote : "이 쇼핑몰은 직접 내려 주세요.")}${listing?.externalId ? ` · 상품번호 ${escapeHtml(listing.externalId)}` : ""}</small></span><em class="${channelId === "smartstore" ? "del" : "stop"}">${escapeHtml(rule ? rule.remove : "직접 처리")}</em></div>`;
  }).join("");
  openModal(`<div class="delete-product-modal"><span class="delete-icon">🗑</span><h2>이 상품을 삭제할까요?</h2><p><b>${escapeHtml(sellerProductTitle(item, product))}</b><br>${live.length ? "지금 판매중인 쇼핑몰에도 자동으로 반영돼요." : "아직 쇼핑몰에 올리지 않은 상품이라 두고에서만 지워져요."}</p>
    ${rows ? `<div class="delete-channel-list">${rows}</div>` : ""}
    <small class="plan-note">삭제해도 상품 소싱에서 다시 PICK할 수 있어요. 이미 들어온 주문은 그대로 남아요.</small>
...
function channelApiLogModal(sellerProductId) {
  ...
  const item = state.sellerProducts.find(entry => entry.id === sellerProductId);
  openModal(`<h2>쇼핑몰 연동 기록</h2><p>${escapeHtml(item ? sellerProductTitle(item, productOf(item.productId)) : sellerProductId)} · 두고가 쇼핑몰 API로 보낸 요청이에요.</p><div class="api-log-list">${entries.length ? channelApiLogRows(entries) : `<div class="empty">아직 기록이 없어요.</div>`}</div><div class="modal-actions"><button type="button" class="secondary-button" data-close-modal>닫기</button></div>`);
}
...
function channelPolicyPresets(channelId) {
  /* 쿠팡은 출고지·반품지 코드, 네이버는 출고지·반품지 주소록 ID가 상품 등록에 필요하다 (실제로는 각 API가 돌려주는 값) */
  const base = { returnFee: 3000, exchangeFee: 6000, jejuFee: 3000, carrier: "CJ대한통운", address: "경기도 광주시 도척면 두고물류센터", zipCode: "12799", outboundCode: 4458021, returnCenterCode: "1000556677", shippingAddressId: 100101, returnAddressId: 100102 };
  return ({
    smartstore: [
...
function channelConnectModal(channelId, { force = false } = {}) {
  const channel = sellerChannels().find(item => item.id === channelId);
  if (!channel) return;
  if (!force && channel.status === "connected" && channel.api) return channelDetailModal(channelId);
  const supported = channelApiSupported(channel.id);
...
function channelApiInfoRows(channel) {
  const api = channel.api || {};
  const row = (label, value) => `<div><dt>${label}</dt><dd>${escapeHtml(value || "-")}</dd></div>`;
  if (channel.id === "smartstore") return row("연동 방식", "API 대행사 연동") + row("판매자 아이디", api.sellerId);
  if (channel.id === "coupang") return row("업체코드", api.sellerId) + row("Wing 아이디", api.vendorUserId) + row("Access Key", api.apiKeyMasked) + row("Secret Key", api.secretMasked || "••••");
  return row("판매자 ID", api.sellerId) + row("API 키", api.apiKeyMasked) + row("시크릿 키", api.secretMasked || "••••");
}
...
function autoIssueTracking(orderId, showResult = true) {
  ...
  audit("송장 자동발급·출력", `${order.id} · ${order.carrier} ${order.tracking} · 공급사 계정 설정으로 자동 생성, 두고 셀러 즉시 반영, 쇼핑몰 전송 대기(자동 전송 설정 시 바로 전송)`, "pending", "tracking");
  saveState(); render(); updateAccountUI();
...
function memberDetailModal(id) {
  ...
  openModal(`<h2>사업자 회원 상세</h2><p>${memberStatusChip(member.status)} · ${escapeHtml(roleSummary)}${roles.length > 1 ? ' <span class="dual-role-badge">복수역할</span>' : ""}</p>
    <div class="member-detail-grid">
      ...
      <div><span>사업자 확인</span><b>${member.businessVerified ? `진위 확인 완료 · ${escapeHtml(member.businessTaxType || "일반과세자")}${member.openDate ? ` · 개업 ${escapeHtml(member.openDate)}` : ""}` : "확인 기록 없음"}</b></div><div><span>휴대폰 인증</span><b>${member.phoneVerified ? "인증 완료" : "기록 없음"}</b></div><div><span>통신판매업 신고번호</span><b>${escapeHtml(member.mailOrderNo || "미입력")}</b></div><div><span>가입 방식</span><b>${member.kakaoId ? "카카오(종료된 방식)" : "이메일 가입"}</b></div><div><span>세금계산서 이메일</span><b>${escapeHtml(member.taxInvoiceEmail || member.email || "-")}</b></div>
...
document.addEventListener("click", event => {
  ...
  if (optionAdd) { const rows = optionAdd.closest(".option-editor")?.querySelector("[data-option-rows]"); if (rows) { rows.insertAdjacentHTML("beforeend", supplierOptionRow()); rows.lastElementChild?.querySelector('input[name="optionName"]')?.focus(); } return; }
  const optionRemove = event.target.closest("[data-option-remove]");
  if (optionRemove) { const rows = optionRemove.closest("[data-option-rows]"); if (rows && rows.children.length > 1) optionRemove.closest(".option-editor-row").remove(); else showToast("옵션은 하나 이상 있어야 해요. 옵션을 안 쓰려면 ‘옵션 사용’을 꺼 주세요."); return; }
  const studioJump = event.target.closest("[data-studio-jump]");
  if (studioJump) { document.getElementById(studioJump.dataset.studioJump)?.scrollIntoView({ behavior: "smooth", block: "start" }); return; }
  const titleWord = event.target.closest("[data-title-word]");
  if (titleWord) {
    const input = document.getElementById("studioTitle");
    const word = `[${titleWord.dataset.titleWord}]`;
    if (input && !input.value.includes(word)) { input.value = `${word} ${input.value}`.slice(0, 50); input.dispatchEvent(new Event("input", { bubbles: true })); }
    return;
  }
  const titleOriginal = event.target.closest("[data-title-original]");
  if (titleOriginal) { const input = document.getElementById("studioTitle"); if (input) { input.value = titleOriginal.dataset.titleOriginal.slice(0, 50); input.dispatchEvent(new Event("input", { bubbles: true })); } return; }
  const brandTab = event.target.closest("[data-brand-tab]");
  if (brandTab) { brandTab.parentElement.querySelectorAll("[data-brand-tab]").forEach(tab => tab.classList.toggle("active", tab === brandTab)); filterBrandBanners(); return; }
  ...
  if (action === "toggle-edit-mode") {
    if (!editMode) {
      ...
      state.sellerDashboard = { layout: [...sellerDashboardLayout()], layoutVersion: DASHBOARD_LAYOUT_VERSION };
      saveState();
      editMode = false;
  ...
  if (action === "manage-alert-plan") { closeNotificationDropdown(); if (activeRole === "seller") { activeMenuIndex = menuIndexOf("요금제", "seller"); render(); updateAccountUI(); window.scrollTo(0, 0); return; } alertPlanModal(); return; }
  ...
  if (action === "category-jump") {
    const [group, mid, sub, detail] = String(target.dataset.path || "").split("|");
    closeModal(); activeMenuIndex = menuIndexOf("상품 소싱");
    sellerCategoryGroup = group || "전체"; sellerCategory = mid || "전체보기"; sellerCategorySub = sub || "전체보기"; sellerCategoryDetail = detail || "전체보기";
    render(); updateAccountUI(); document.querySelector(".market-toolbar")?.scrollIntoView({ behavior: "smooth", block: "start" }); return;
  }
  if (action === "category-crumb") {
    const level = Number(target.dataset.level || 0);
    if (level === 0) sellerCategoryGroup = "전체";
    if (level <= 1) sellerCategory = "전체보기";
    if (level <= 2) sellerCategorySub = "전체보기";
    if (level <= 3) sellerCategoryDetail = "전체보기";
    render(); updateAccountUI(); return;
  }
  if (action === "category-chip") {
    const level = Number(target.dataset.level || 1); const value = target.dataset.value || "";
    if (level === 1) { sellerCategoryGroup = value; sellerCategory = "전체보기"; sellerCategorySub = "전체보기"; sellerCategoryDetail = "전체보기"; }
    if (level === 2) { sellerCategory = value; sellerCategorySub = "전체보기"; sellerCategoryDetail = "전체보기"; }
    if (level === 3) { sellerCategorySub = value; sellerCategoryDetail = "전체보기"; }
    if (level === 4) sellerCategoryDetail = sellerCategoryDetail === value ? "전체보기" : value;
    render(); updateAccountUI(); return;
  }
  ...
  if (action === "market-preset") { if (target.dataset.category) sellerCategoryGroup = "식품"; sellerCategory = target.dataset.category || "전체보기"; sellerCategorySub = "전체보기"; sellerCategoryDetail = "전체보기"; sellerShippingFilter = target.dataset.filter || "all"; sellerCountry = target.dataset.country || "전체 국가"; activeMenuIndex = menuIndexOf("상품 소싱"); render(); updateAccountUI(); requestAnimationFrame(() => document.getElementById("marketProductGrid")?.scrollIntoView({ behavior:"smooth", block:"start" })); return; }
  if (action === "open-brand-directory") { closeModal(); activeMenuIndex = menuIndexOf("브랜드 소싱"); sellerBrand = "전체 브랜드"; render(); updateAccountUI(); window.scrollTo({ top: 0, behavior: "smooth" }); return; }
  if (action === "close-brand-directory") { closeModal(); activeMenuIndex = menuIndexOf("상품 소싱"); render(); updateAccountUI(); window.scrollTo({ top: 0, behavior: "smooth" }); return; }
  if (action === "select-brand") { closeModal(); activeMenuIndex = menuIndexOf("브랜드 소싱"); sellerBrand = target.dataset.brand || "전체 브랜드"; render(); updateAccountUI(); window.scrollTo({ top: 0, behavior: "smooth" }); return; }
  if (action === "clear-market-filter") {
    const filter = target.dataset.filter;
    if (filter === "category") { sellerCategoryGroup = "전체"; sellerCategory = "전체보기"; sellerCategorySub = "전체보기"; sellerCategoryDetail = "전체보기"; }
    if (filter === "shipping") sellerShippingFilter = "all";
    if (filter === "country") sellerCountry = "전체 국가";
    if (filter === "search") sellerProductSearch = "";
    render(); updateAccountUI();
    if (filter === "search") document.getElementById("sellerCatalogSearch")?.focus({ preventScroll: true });
    return;
  }
  if (action === "reset-market-filter") { sellerCategory = "전체보기"; sellerCategoryGroup = "전체"; sellerCategorySub = "전체보기"; sellerCategoryDetail = "전체보기"; sellerShippingFilter = "all"; sellerCountry = "전체 국가"; sellerProductSearch = ""; render(); updateAccountUI(); return; }
  if (action === "set-market-view") { marketViewMode = target.dataset.view || "grid"; render(); updateAccountUI(); return; }
  if (action === "reset-onsale-filter") { onSaleSearch = ""; onSaleDateFrom = ""; onSaleDateTo = ""; onSaleCategoryFilter = "전체"; onSalePage = 1; render(); updateAccountUI(); return; }
  ...
  if (action === "register-master-product") {
    ...
    item.masterRegisteredAt = "방금 전";
    audit("마스터 상품 등록", `${item.id} · ${sellerProductTitle(item)}을(를) 마스터 상품으로 등록했습니다.`, "done", "product");
    saveState(); closeModal(); activeMenuIndex = menuIndexOf("마스터 상품"); render(); updateAccountUI(); window.scrollTo({ top: 0, behavior: "smooth" });
    showToast("마스터 상품에 등록했어요. 이제 ‘상품 전송’을 누르면 쇼핑몰에 올라가요.");
    return;
  }
  if (action === "open-picked-product") {
    ...
    closeModal();
    const stage = sellerItemStage(picked);
    const step = { live: [menuIndexOf("판매중 상품"), "판매중 상품"], master: [menuIndexOf("마스터 상품"), "마스터 상품"], ready: [menuIndexOf("승인 완료"), "승인 완료"] }[stage] || [menuIndexOf("승인 대기"), "승인 대기"];
  ...
  if (action === "supplier-approve-pick") {
    ...
    render(); updateAccountUI(); showToast("승인했어요. 위탁셀러의 ‘승인 완료’ 상품으로 넘어갔어요."); return;
  }
  ...
  if (action === "rerequest-pick") {
    ...
    if (supplierAutoApprove(product?.supplierLoginId)) { approvePickRequest(item, true); saveState(); activeMenuIndex = menuIndexOf("승인 완료"); render(); updateAccountUI(); window.scrollTo({top:0,behavior:"smooth"}); showToast("자동 승인 공급사라 바로 승인됐어요. ‘승인 완료’에서 꾸미고 등록하세요."); return; }
  ...
  if (action === "supplier-approve-all-picks") {
    const requests = supplierPickRequests();
    requests.forEach(item => approvePickRequest(item));
    saveState(); render(); updateAccountUI(); showToast(`${requests.length}건을 모두 승인했어요.`); return;
  }
  if (action === "toggle-supplier-auto-approve") {
    const loginId = currentAccount.loginId;
    state.supplierSettings = state.supplierSettings || {};
    state.supplierSettings[loginId] = { ...(state.supplierSettings[loginId] || {}), autoApprove: !supplierAutoApprove(loginId) };
    const enabled = supplierAutoApprove(loginId);
    let approvedNow = 0;
    if (enabled) { const requests = supplierPickRequests(loginId); requests.forEach(item => approvePickRequest(item, true)); approvedNow = requests.length; }
    audit("PICK 자동 승인 설정", `${currentAccount.company || loginId} · 자동 승인 ${enabled ? "켜짐" : "꺼짐"}`, "done", "product");
    saveState(); render(); updateAccountUI(); showToast(enabled ? `자동 승인을 켰어요.${approvedNow ? ` 대기 중이던 ${approvedNow}건도 바로 승인했습니다.` : ""} 이제 PICK하면 바로 승인됩니다.` : "자동 승인을 껐어요. PICK 요청을 직접 확인하고 승인합니다."); return;
  }
  if (action === "sync-channel-listing") {
    ...
    if (item && detail) {
      ...
      if (Array.isArray(detail.options) && detail.options.length) { item.optionPrices = item.optionPrices || {}; detail.options.forEach(option => { item.optionPrices[option.optionId] = Number(option.salePrice) || item.optionPrices[option.optionId]; }); }
      item.channelSyncStatus = item.channelSyncStatus || {};
  ...
  if (action === "channel-api-log") { channelApiLogModal(id); return; }
  if (action === "delete-seller-product") { sellerProductDeleteModal(id); return; }
  if (action === "confirm-delete-seller-product") {
    const item = state.sellerProducts.find(entry => entry.id === id && entry.sellerLoginId === currentAccount.loginId);
    if (!item) return closeModal();
    const live = channelsOnMarket(item).filter(channelApiSupported);
    const title = sellerProductTitle(item, productOf(item.productId));
    Promise.all(live.map(channelId => runChannelOp(item, channelId, "remove", { silent: true }))).then(results => {
      const index = state.sellerProducts.indexOf(item);
      if (index >= 0) state.sellerProducts.splice(index, 1);
      audit("마스터 상품 삭제", `${item.id} · ${title}${live.length ? ` · ${live.map(channelId => `${CHANNEL_RULES[channelId].name} ${results[live.indexOf(channelId)]?.deleted ? "삭제" : "판매중지"}`).join(", ")}` : ""}`, "done", "product");
      saveState(); closeModal(); render(); updateAccountUI();
      showToast(live.length ? `삭제했어요. ${live.map(channelId => `${CHANNEL_RULES[channelId].name}: ${results[live.indexOf(channelId)]?.deleted ? "삭제됨" : results[live.indexOf(channelId)] ? "판매중지" : "실패(연동 기록 확인)"}`).join(" · ")}` : "마스터 상품에서 삭제했어요.");
    });
    return;
  }
  ...
  if (action === "push-all-tracking") {
    const result = pushAllPendingTracking("manual");
    saveState(); render(); updateAccountUI();
    showToast(result.count ? `송장 ${result.count}건을 ${result.channels.join(", ")}에 보냈어요.` : "보낼 송장이 없어요.");
    return;
  }
  if (action === "toggle-auto-tracking") {
    const enable = !autoTrackingPushEnabled();
    const connected = sellerChannels().filter(channel => channel.status === "connected");
...
document.getElementById("loginForm").addEventListener("submit", event => {
  ...
  if (account?.kakaoId && !account.password) {
    document.getElementById("loginError").textContent = "카카오 간편 로그인이 종료됐어요. ‘아이디·비밀번호 찾기’에서 비밀번호를 만들어 주세요.";
    return;
  }
...
document.getElementById("signupForm").addEventListener("submit", event => {
  event.preventDefault();
  for (const step of [1, 2, 3, 4]) if (!validateSignupStep(step)) { setSignupStep(step); validateSignupStep(step); return; }
  const form = event.currentTarget;
  const data = new FormData(form);
  const loginId = String(data.get("email") || "").trim().toLowerCase();
  const role = data.get("role") === "supplier" ? "supplier" : "seller";
  const docsLater = Boolean(data.get("docsLater"));
  const company = String(data.get("company") || "").trim();
  const member = {
    id: nextMemberId(), loginId, password: String(data.get("password") || ""), role,
    roleLabel: role === "supplier" ? "공급사" : "위탁셀러", name: company, company,
    representative: String(data.get("representative") || "").trim(), businessNo: formatBusinessNo(data.get("businessNo")), openDate: String(data.get("openDate") || ""),
    businessVerified: true, businessTaxType: "일반과세자", mailOrderNo: String(data.get("mailOrderNo") || "").trim(),
    contact: String(data.get("contact") || ""), phoneVerified: true, email: loginId, taxInvoiceEmail: String(data.get("taxInvoiceEmail") || "").trim() || loginId,
    referralCode: String(data.get("referralCode") || "").trim().toUpperCase(), marketingConsent: Boolean(data.get("marketingConsent")),
    businessFile: docsLater ? "" : signupFileName(form.elements.businessFile), bankbookFile: docsLater ? "" : signupFileName(form.elements.bankbookFile),
    bankName: docsLater ? "" : String(data.get("bankName") || ""), bankAccountHolder: docsLater ? "" : String(data.get("bankAccountHolder") || "").trim(), bankAccountNumber: docsLater ? "" : String(data.get("bankAccountNumber") || "").replace(/[^\d-]/g, ""),
    status: "approved", docsPending: docsLater, appliedAt: "방금 전", approvedAt: "가입 즉시 이용", rejectReason: "", roles: [role]
  };
  state.members.push(member);
  state.logs.unshift({ id: Date.now(), type: "member", title: "신규 회원 가입 · 즉시 이용", detail: `${member.company} · ${member.roleLabel} · 사업자 진위 확인 완료${docsLater ? " · 서류는 나중에 제출" : " · 서류 제출 완료"}`, time: "방금 전", state: "done" });
  state.members.filter(item => item.role === "master").forEach(master => pushNotification(master.loginId, "master", "member", "신규 회원 가입", `${member.company} · ${member.roleLabel} · ${member.businessNo}`));
  saveState();
  clearSignupDraft();
  localStorage.setItem(REMEMBER_KEY, loginId);
  document.getElementById("signupView").hidden = true;
  showApp(loginId);
  showToast(`가입 완료! ${member.company}님, 지금 바로 두고를 쓸 수 있어요.${docsLater ? " 정산 계좌·서류는 나중에 등록해 주세요." : ""}`);
});
...
document.addEventListener("change", event => {
  if (event.target.id === "studioThumbInput" || event.target.id === "studioBlockImageInput") {
  ...
  if (event.target.id === "externalOrderSource") {
    ...
    if (option && form) {
      form.elements.externalProductName.value = option.dataset.name || "";
      form.elements.externalProductCode.value = option.value;
      form.elements.channel.value = option.dataset.channel || "쿠팡";
      form.elements.amount.value = option.dataset.amount || 29900;
      const customs = form.querySelector("[data-customs-field]");
      if (customs) { const needed = option.dataset.overseas === "true"; customs.style.display = needed ? "" : "none"; customs.querySelector("input").required = needed; }
    }
    return;
  }
...
document.addEventListener("submit", event => {
  ...
  if (form.id === "importForm") {
    ...
    if (!source) return;
    let pricing;
    if (hasOptions(source)) { pricing = readOptionPrices(form, source); if (pricing.message) return showToast(pricing.message); }
    else { const salePrice = Number(data.salePrice || source.recommended); if (salePrice < Number(source.supply)) return showToast(`판매가가 공급가(${money(source.supply)})보다 낮아요.`); pricing = { salePrice }; }
    const customTitle = source.name;
    ...
    if (autoApproved) {
      ...
      saveState(); closeModal(); activeMenuIndex = menuIndexOf("승인 완료"); render(); updateAccountUI(); window.scrollTo({top:0,behavior:"smooth"}); showToast("자동 승인 공급사라 바로 내 상품이 됐어요! 꾸미고 ‘마스터 상품 등록’을 눌러 주세요.");
    } else {
      pushNotification(source.supplierLoginId, "supplier", "approval", "PICK 승인 요청이 도착했습니다", `${currentAccount.company || currentAccount.loginId} · ${source.id} ${source.name}${hasOptions(source) ? ` · 옵션 ${Object.values(pricing.optionEnabled).filter(Boolean).length}개` : ""}`);
      audit("두고 상품 PICK", `${source.id} 상품을 PICK했습니다. ${source.supplier} 공급사 승인 후 승인 완료로 이동합니다.`, "done", "product");
      saveState(); closeModal(); activeMenuIndex = menuIndexOf("승인 대기"); pickStatusFilter = "pending"; expandedSellerProductId = newId; render(); updateAccountUI(); window.scrollTo({top:0,behavior:"smooth"}); showToast("PICK 완료! 공급사가 승인하면 ‘승인 완료’로 넘어가요.");
    }
    return;
  }
  if (form.id === "studioForm") {
    const item = state.sellerProducts.find(entry => entry.id === form.dataset.id && entry.sellerLoginId === currentAccount.loginId);
    const source = productOf(item?.productId);
    if (!item || !source) return showToast("수정할 상품을 찾지 못했어요.");
    const intent = event.submitter?.value || "save";
    const title = String(data.customTitle || "").trim();
    if (!title) { document.getElementById("studioTitle")?.focus(); return showToast("상품명을 입력해 주세요."); }
    ...
    audit(intent === "register" ? "상품 꾸미기 · 마스터 상품 등록" : "내 상품 수정", `${item.id} · 원본 ${item.productId} 유지 · ${oldTitle} → ${item.customTitle} · ${sellerItemPriceLabel(item, source)}`, "done", "product");
    studioDraft = null;
    saveState(); closeModal();
  ...
  if (form.id === "externalOrderForm") {
    ...
    const resolved = resolveMappingForCode(externalProductCode);
    const mappedProduct = resolved?.product || null;
    const customsCode = normalizeCustomsCode(data.personalCustomsCode);
    if (productNeedsCustoms(mappedProduct) && !isValidCustomsCode(customsCode)) return showToast("해외직구 상품이에요. 개인통관고유부호(P+숫자 12자리)를 정확히 입력해 주세요.");
    state.orders.unshift({
      ...
      ...(resolved?.option ? { optionId: resolved.option.id, optionName: resolved.option.name } : {}),
      paymentStatus: "pending",
      paymentMethod: "",
      supplyTotal: mappedProduct ? Number(resolved?.option ? resolved.option.supply : mappedProduct.supply) * Number(data.qty || 1) : 0,
      forwardedAt: "",
      ...
      personalCustomsCode: productNeedsCustoms(mappedProduct) ? customsCode : (customsCode && isValidCustomsCode(customsCode) ? customsCode : ""),
      qty: Number(data.qty),
    ...
    if (mappedProduct) audit("외부 주문 자동 연결", `${orderId} · ${data.channel} · ${data.externalProductName} · ${resolved.type === "auto" ? "두고 전송 상품" : "직접 연결 상품"}으로 ${mappedProduct.id} 자동 연결`, "done", "order");
    else audit("외부 주문 수집", `${orderId} · ${data.channel} · ${data.externalProductName} · 두고 상품 연결 대기`, "pending", "order");
    saveState(); closeModal();
  ...
  if (form.id === "orderPaymentForm") {
    ...
    if (productNeedsCustoms(product)) {
      const customsCode = normalizeCustomsCode(data.personalCustomsCode || order.personalCustomsCode);
      if (!isValidCustomsCode(customsCode)) return showToast("개인통관고유부호(P+숫자 12자리)를 정확히 입력해야 공급사로 보낼 수 있어요.");
  ...
  if (form.id === "productChannelsForm") {
    ...
    const previousLive = liveChannelIds(item);
    ...
    channels.filter(channelApiSupported).forEach(channelId => {
      const listing = channelListing(item, channelId);
      const op = previousLive.includes(channelId) ? "sync" : listing && !listing.deleted && listing.externalId ? "resume" : "register";
      runChannelOp(item, channelId, op, { stock: productOf(item.productId)?.stock });
    });
    previousLive.filter(channelId => !channels.includes(channelId) && channelApiSupported(channelId)).forEach(channelId => runChannelOp(item, channelId, "stop"));
  ...
  if (form.id === "stopChannelForm") {
    ...
    runChannelOp(item, channelId, reason === "품절" ? "soldout" : reason === "상품중지" ? "stop" : "hide");
  ...
  if (form.id === "stockForm") {
    ...
    /* 공급사 재고가 0이 되면 이 상품을 파는 모든 셀러의 쇼핑몰을 품절로, 다시 채우면 판매 재개 */
    if ((oldStock > 0 && nextStock <= 0) || (oldStock <= 0 && nextStock > 0)) {
      const op = nextStock <= 0 ? "soldout" : "resume";
      state.sellerProducts.filter(entry => entry.productId === product.id).forEach(entry => sellerChannels(entry.sellerLoginId).filter(channel => channelApiSupported(channel.id) && ((entry.channels || []).includes(channel.id) || entry.channelListings?.[channel.id]?.status === "품절")).forEach(channel => { if (op === "resume" && entry.channelListings?.[channel.id]?.status !== "품절") return; runChannelOp(entry, channel.id, op, { stock: nextStock, silent: true }); }));
    }
  ...
  if (form.id === "simulateOrderForm") {
    ...
    const sampleOption = data.optionId ? productOptionOf(product, data.optionId) : null;
    const sampleRow = sampleOption ? sellerOptionRows(sellerProduct, product).find(row => row.id === sampleOption.id) : null;
    if (qty > (sampleOption ? Number(sampleOption.stock || 0) : product.stock)) {
  ...
  if (form.id === "channelConnectForm") {
    ...
    const fields = channelApiFields(channel.id);
    const creds = Object.fromEntries(fields.map(([name]) => [name, String(data[name] || "").trim()]));
    const missing = fields.find(([name]) => creds[name].length < 3);
    ...
    if (CHANNEL_SERVER && channelApiSupported(channel.id)) {
      const button = form.querySelector('button[type="submit"]');
  ...
  if (form.id === "memberDocsForm") {
    ...
    const bankbookFile = signupFileName(form.elements.bankbookFile) || member.bankbookFile;
    if (!businessFile) return showToast("사업자등록증을 올려 주세요.");
    if (!data.bankName || !String(data.bankAccountHolder || "").trim() || String(data.bankAccountNumber || "").replace(/\D/g, "").length < 8) return showToast("은행, 예금주, 계좌번호를 넣어 주세요.");
    if (!bankbookFile) return showToast("통장 사본을 올려 주세요.");
    Object.assign(member, { businessFile, bankbookFile, bankName: data.bankName, bankAccountHolder: data.bankAccountHolder.trim(), bankAccountNumber: data.bankAccountNumber.replace(/[^\d-]/g, ""), docsPending: false, docsSubmittedAt: new Date().toISOString() });
    Object.assign(currentAccount, { docsPending: false, businessFile, bankbookFile });
    audit("정산 계좌·서류 등록", `${member.company} · 사업자등록증·통장 사본 등록`, "done", "member");
    saveState(); closeModal(); render(); updateAccountUI(); return showToast("정산 계좌와 서류를 등록했어요.");
  }
  if (form.id === "supplierApplicationForm") {
    ...
    const owner = memberByLogin(currentAccount.loginId);
    if (owner) { owner.roles = [...new Set([...(owner.roles || [owner.role]), "seller", "supplier"])]; owner.supplierCompany = application.company; }
    currentAccount.roles = [...new Set([...(currentAccount.roles || [currentAccount.role]), "seller", "supplier"])]; currentAccount.supplierCompany = application.company;
    pushNotification("admin", "master", "approval", "공급사 모드를 시작했어요", `${application.company} · ${member?.company || currentAccount.loginId}`, ["운영센터"]);
    audit("공급사 모드 즉시 시작", `${member?.company || currentAccount.loginId} · ${application.company} · 심사 없이 바로 사용`, "done", "member");
    saveState(); closeModal(); userSwitchModal(); updateAccountUI(); showToast("공급사 모드를 바로 쓸 수 있어요. 아래에서 공급사를 눌러 전환하세요.");
  }
...
/* ===== 사업자 회원가입 (이메일 · 휴대폰 인증 · 사업자 진위확인 · 서류) =====
   두고는 사업자만 이용하므로 간편 소셜 가입 대신, 짧은 4단계로 필요한 정보를 한 번에 받는다.
   서류가 당장 없으면 '나중에 올리기'로 가입을 먼저 끝내고, 로그인하면 가입 진행 현황 화면에서 이어서 올린다. */
const REFUND_BANKS = ["국민은행", "신한은행", "우리은행", "하나은행", "농협은행", "기업은행", "카카오뱅크", "토스뱅크", "케이뱅크", "새마을금고", "우체국", "SC제일은행", "iM뱅크(대구)", "부산은행", "경남은행", "광주은행", "전북은행", "수협은행", "신협"];
const SIGNUP_DRAFT_KEY = "doogo-signup-draft";
const SIGNUP_DRAFT_FIELDS = ["email", "contact", "businessNo", "representative", "openDate", "company", "mailOrderNo", "bankName", "bankAccountHolder", "bankAccountNumber", "taxInvoiceEmail", "referralCode", "phoneVerified", "businessVerified"];
let signupPhoneCode = "";
/* 국세청 사업자등록번호 검증 규칙(체크섬) */
function isValidBusinessNo(value) {
...
function formatBusinessNo(value) { const d = String(value || "").replace(/\D/g, "").slice(0, 10); return d.length > 5 ? `${d.slice(0, 3)}-${d.slice(3, 5)}-${d.slice(5)}` : d.length > 3 ? `${d.slice(0, 3)}-${d.slice(3)}` : d; }
function formatPhone(value) { const d = String(value || "").replace(/\D/g, "").slice(0, 11); return d.length > 7 ? `${d.slice(0, 3)}-${d.slice(3, d.length - 4)}-${d.slice(-4)}` : d.length > 3 ? `${d.slice(0, 3)}-${d.slice(3)}` : d; }
function signupDraft() { try { return JSON.parse(localStorage.getItem(SIGNUP_DRAFT_KEY) || "null"); } catch { return null; } }
function saveSignupDraft() {
  const form = document.getElementById("signupForm");
  if (!form || document.getElementById("signupView").hidden) return;
  const draft = { role: form.elements.role.value, step: signupStep, savedAt: Date.now() };
  SIGNUP_DRAFT_FIELDS.forEach(name => { if (form.elements[name]) draft[name] = form.elements[name].value; });
  try { localStorage.setItem(SIGNUP_DRAFT_KEY, JSON.stringify(draft)); } catch {}
}
function clearSignupDraft() { try { localStorage.removeItem(SIGNUP_DRAFT_KEY); } catch {} }
function restoreSignupDraft(role) {
  const draft = signupDraft();
  const note = document.querySelector("[data-signup-draft-note]");
  if (!draft || draft.role !== role || !draft.email) { if (note) note.hidden = true; return 1; }
  const form = document.getElementById("signupForm");
  SIGNUP_DRAFT_FIELDS.forEach(name => { if (form.elements[name] && draft[name] !== undefined) form.elements[name].value = draft[name]; });
  if (draft.phoneVerified) setPhoneStatus(true);
  if (draft.businessVerified) showBusinessResult(true, form);
  if (note) note.hidden = false;
  return Math.min(Number(draft.step || 1), 2);
}
function setPhoneStatus(verified, message = "") {
  ...
  if (verified) { row.hidden = true; send.textContent = "✓ 인증 완료"; send.classList.add("done"); form.elements.contact.readOnly = true; }
  else { send.textContent = "인증번호 받기"; send.classList.remove("done"); form.elements.contact.readOnly = false; }
  if (status) status.textContent = message;
}
function showBusinessResult(ok, form, message = "") {
  const box = form.querySelector("[data-business-result]");
  form.elements.businessVerified.value = ok ? "yes" : "";
  box.hidden = false;
  box.className = `signup-verify-result ${ok ? "ok" : "fail"}`;
  box.innerHTML = ok ? `<b>✓ 확인된 사업자예요</b><span>계속사업자 · 일반과세자</span>` : `<b>확인하지 못했어요</b><span>${escapeHtml(message)}</span>`;
}
function fillBankSelect(form) {
  const select = form.querySelector("[data-bank-select]");
  if (select && !select.options.length) select.innerHTML = `<option value="">은행 선택</option>${REFUND_BANKS.map(bank => `<option>${bank}</option>`).join("")}`;
}
function signupFileName(input) { return input?.files?.[0]?.name || input?.dataset.kept || ""; }
function signupStatusSteps(member) {
  const docsDone = Boolean(member.businessFile && (member.bankbookFile || member.bankAccountNumber));
  return [
    { label: "계정 만들기", done: true, note: member.loginId },
    { label: "사업자 확인", done: Boolean(member.businessVerified || member.status === "approved"), note: member.businessVerified ? `${member.businessNo} · 진위 확인됨` : member.businessNo || "" },
    { label: "서류 · 정산 계좌", done: docsDone, note: docsDone ? `사업자등록증 ${member.businessFile ? "✓" : "–"} · 통장 ${member.bankbookFile ? "✓" : member.bankAccountNumber ? "계좌 입력" : "–"}` : "사업자등록증과 통장 사본이 필요해요" },
    { label: "두고 승인", done: member.status === "approved", note: member.status === "rejected" ? `반려: ${member.rejectReason || "사유 확인 필요"}` : member.status === "pending" ? "검토 중이에요 · 보통 1영업일 안에 끝나요" : "서류를 올리면 검토를 시작해요" }
  ];
}
/* 가입 신청 직후 · 승인 전 로그인 시 보여주는 '가입 진행 현황' */
function showSignupStatus(member, justApplied = false) {
  document.getElementById("loginView").hidden = true;
  document.getElementById("partnerLoginView").hidden = true;
  document.getElementById("appView").hidden = true;
  document.getElementById("signupView").hidden = false;
  ...
  document.getElementById("signupComplete").dataset.memberId = member.id;
  const steps = signupStatusSteps(member);
  const needDocs = false;
  const title = member.status === "rejected" ? "이용이 제한된 계정이에요" : !steps[2].done ? (justApplied ? "가입이 됐어요! 서류만 남았어요" : "남은 서류를 올려 주세요") : justApplied ? "가입 신청이 접수됐어요" : "두고에서 검토하고 있어요";
  const sub = member.status === "rejected" ? "아래 사유를 확인하고 두고 고객센터(카카오톡 채널 ‘두고’)로 문의해 주세요." : !steps[2].done ? "서류를 올리면 바로 검토를 시작해요. 지금 휴대폰으로 찍어서 올려도 돼요." : `승인되면 <b>${escapeHtml(member.contact || "휴대폰")}</b>으로 문자를 보내드려요. 승인 후 바로 로그인할 수 있어요.`;
  document.getElementById("signupStatusBody").innerHTML = `<span class="complete-icon ${member.status === "rejected" ? "warn" : ""}">${member.status === "rejected" ? "!" : "✓"}</span><h2>${title}</h2><p><b>${escapeHtml(member.company)}</b> · ${escapeHtml(member.roleLabel || "위탁셀러")}<br>${sub}</p>
    <ol class="signup-status-list">${steps.map((step, index) => `<li class="${step.done ? "done" : index === steps.findIndex(item => !item.done) ? "now" : ""}"><i>${step.done ? "✓" : index + 1}</i><span><b>${step.label}</b><small>${escapeHtml(step.note)}</small></span></li>`).join("")}</ol>
    ${needDocs ? `<form id="signupDocsForm" class="signup-docs-form" data-member="${member.id}" novalidate>
      <div class="signup-field full"><label>사업자등록증 ${member.businessFile && member.status !== "rejected" ? `<small>제출됨 · ${escapeHtml(member.businessFile)}</small>` : "<em>필수</em>"}</label><label class="signup-file"><input name="businessFile" type="file" accept="image/*,.pdf" data-signup-file><span data-file-label>${member.businessFile && member.status !== "rejected" ? "✓ 다시 올리려면 누르세요" : "📄 사진 찍기 또는 파일 선택"}</span></label></div>
      <div class="signup-fields"><div class="signup-field"><label>정산·환불 은행</label><select name="bankName"><option value="">은행 선택</option>${REFUND_BANKS.map(bank => `<option ${member.bankName === bank ? "selected" : ""}>${bank}</option>`).join("")}</select></div><div class="signup-field"><label>예금주</label><input name="bankAccountHolder" value="${escapeHtml(member.bankAccountHolder || "")}" placeholder="상호 또는 대표자명"></div><div class="signup-field full"><label>계좌번호</label><input name="bankAccountNumber" inputmode="numeric" value="${escapeHtml(member.bankAccountNumber || "")}" placeholder="- 없이 숫자만"></div></div>
      <div class="signup-field full"><label>통장 사본 ${member.bankbookFile && member.status !== "rejected" ? `<small>제출됨 · ${escapeHtml(member.bankbookFile)}</small>` : "<em>필수</em>"}</label><label class="signup-file"><input name="bankbookFile" type="file" accept="image/*,.pdf" data-signup-file><span data-file-label>${member.bankbookFile && member.status !== "rejected" ? "✓ 다시 올리려면 누르세요" : "📄 사진 찍기 또는 파일 선택"}</span></label></div>
      <p class="signup-step-error" data-docs-error></p>
      <button class="login-submit" type="submit">서류 제출하고 검토 요청</button></form>` : ""}`;
  window.scrollTo({ top: 0 });
}
function submitSignupDocs(form) {
  const member = state.members.find(item => item.id === form.dataset.member);
  if (!member) return;
  const error = form.querySelector("[data-docs-error]");
  const data = Object.fromEntries(new FormData(form));
  const rejected = member.status === "rejected";
  const businessFile = signupFileName(form.elements.businessFile) || (rejected ? "" : member.businessFile);
  const bankbookFile = signupFileName(form.elements.bankbookFile) || (rejected ? "" : member.bankbookFile);
  if (!businessFile) { error.textContent = "사업자등록증을 올려 주세요."; return; }
  if (!data.bankName || !String(data.bankAccountHolder || "").trim() || String(data.bankAccountNumber || "").replace(/\D/g, "").length < 8) { error.textContent = "정산·환불 받을 은행, 예금주, 계좌번호를 넣어 주세요."; return; }
  if (!bankbookFile) { error.textContent = "통장 사본을 올려 주세요."; return; }
  Object.assign(member, { businessFile, bankbookFile, bankName: data.bankName, bankAccountHolder: data.bankAccountHolder.trim(), bankAccountNumber: data.bankAccountNumber.replace(/[^\d-]/g, ""), status: "pending", rejectReason: "", appliedAt: "방금 전", docsSubmittedAt: new Date().toISOString() });
  state.logs.unshift({ id: Date.now(), type: "member", title: rejected ? "반려 후 서류 재제출" : "가입 서류 제출", detail: `${member.company} · 사업자등록증·통장 사본 제출 → 승인 검토 요청`, time: "방금 전", state: "pending" });
  state.members.filter(item => item.role === "master").forEach(master => pushNotification(master.loginId, "master", "member", "가입 서류가 제출됐어요", `${member.company} · ${member.businessNo} · 승인 검토 필요`));
  saveState();
  showSignupStatus(member);
  showToast("서류를 제출했어요. 두고에서 검토를 시작해요.");
}
document.addEventListener("click", event => {
  const form = event.target.closest?.("#signupForm");
  if (event.target.closest?.("[data-signup-reset]")) {
    clearSignupDraft(); const role = document.getElementById("signupRole").value; showSignup(role, signupReturnTarget); return;
  }
  if (!form) return;
  if (event.target.closest("[data-phone-send]")) {
    if (form.elements.phoneVerified.value) return;
    const phone = form.elements.contact.value.replace(/\D/g, "");
    if (!/^01[016789]\d{7,8}$/.test(phone)) { form.querySelector('[data-signup-step="1"] [data-signup-error]').textContent = "휴대폰 번호를 확인해 주세요."; form.elements.contact.focus(); return; }
    signupPhoneCode = String(Math.floor(100000 + Math.random() * 900000));
    form.querySelector("[data-phone-code-row]").hidden = false;
  ...
  if (event.target.closest("[data-phone-check]")) {
    if (form.elements.phoneCode.value.trim() === signupPhoneCode && signupPhoneCode) { setPhoneStatus(true, ""); saveSignupDraft(); showToast("휴대폰 인증이 끝났어요."); }
    else form.querySelector("[data-phone-status]").textContent = "인증번호가 맞지 않아요. 다시 확인해 주세요.";
    return;
  }
  if (event.target.closest("[data-business-verify]")) {
    const no = form.elements.businessNo.value, rep = form.elements.representative.value.trim(), open = form.elements.openDate.value.replace(/\D/g, "");
    if (!isValidBusinessNo(no)) return showBusinessResult(false, form, "사업자등록번호 10자리를 다시 확인해 주세요. 번호 규칙에 맞지 않아요.");
    if (!rep) return showBusinessResult(false, form, "대표자명을 넣어 주세요.");
    if (open.length !== 8) return showBusinessResult(false, form, "개업일자를 2021-03-15처럼 넣어 주세요.");
    const button = event.target.closest("[data-business-verify]");
    button.disabled = true; button.textContent = "국세청에서 확인하는 중…";
    setTimeout(() => { button.disabled = false; button.textContent = "사업자 진위 확인하기"; showBusinessResult(true, form); saveSignupDraft(); form.elements.company.focus(); }, 700);
    return;
  }
});
document.addEventListener("input", event => {
  const form = event.target.closest?.("#signupForm");
  if (!form) return;
  if (event.target.name === "businessNo") { const pos = event.target.value.length; event.target.value = formatBusinessNo(event.target.value); if (form.elements.businessVerified.value) { form.elements.businessVerified.value = ""; form.querySelector("[data-business-result]").hidden = true; } }
  if (event.target.name === "contact" && !event.target.readOnly) event.target.value = formatPhone(event.target.value);
  if (event.target.name === "openDate") { const d = event.target.value.replace(/\D/g, "").slice(0, 8); event.target.value = d.length > 6 ? `${d.slice(0, 4)}-${d.slice(4, 6)}-${d.slice(6)}` : d.length > 4 ? `${d.slice(0, 4)}-${d.slice(4)}` : d; }
  if (["representative", "openDate"].includes(event.target.name) && form.elements.businessVerified.value) { form.elements.businessVerified.value = ""; form.querySelector("[data-business-result]").hidden = true; }
  saveSignupDraft();
});
document.addEventListener("change", event => {
  const file = event.target.closest?.("[data-signup-file]");
  if (file) { const label = file.closest(".signup-file").querySelector("[data-file-label]"); label.textContent = file.files?.[0] ? `✓ ${file.files[0].name}` : "📄 사진 찍기 또는 파일 선택"; file.closest(".signup-file").classList.toggle("has-file", Boolean(file.files?.[0])); return; }
  if (event.target.matches?.("[data-agree-all]")) { event.target.closest("form").querySelectorAll("[data-agree-item]").forEach(box => { box.checked = event.target.checked; }); return; }
  if (event.target.matches?.("[data-agree-item]")) { const form = event.target.closest("form"); form.querySelector("[data-agree-all]").checked = [...form.querySelectorAll("[data-agree-item]")].every(box => box.checked); return; }
  if (event.target.name === "docsLater" && event.target.closest("#signupForm")) { const form = event.target.closest("form"); form.classList.toggle("docs-later", event.target.checked); form.querySelector('[data-signup-step="3"] [data-signup-error]').textContent = ""; return; }
  if (event.target.closest?.("#signupForm")) saveSignupDraft();
});
document.addEventListener("submit", event => {
  if (event.target.id === "signupDocsForm") { event.preventDefault(); submitSignupDocs(event.target); }
});
